# Seller Partner App — API Spec

Verified against `Backend/src/modules/food/seller/` and `Backend/src/modules/food/orders/`.

## Global

- **Base URL:** `{HOST}/api/v1`
- **Prefix for everything here:** `/food/seller`
- **Auth:** `Authorization: Bearer <accessToken>`, role must be `SELLER`
- **Envelope:** `{ "success": true, "message": "…", "data": … }` / errors `{ "success": false, "message": "…" }`
- **Static files:** `{HOST}/uploads/...`
- **Caching:** the public read endpoints (`/sellers`, `/sellers/:id`, menu, timings, addons, categories) are server-cached 5–10 min. Writes from the dashboard invalidate them automatically — but a partner's own edit may take a beat to appear on the public route.

---

## 1. Auth

### `POST /food/auth/seller/request-otp`
`{ "phone": "9876543210" }`

### `POST /food/auth/seller/verify-otp`
```json
...
### `POST /food/seller/register` — `multipart/form-data`
...
```json
{
  ...
  "pureVegSeller": "true",          // required; accepts true/false/1/0/yes/no
...
→ 201, `data` = the created seller.

### `POST /food/seller/onboarding-fee/order`
`{ "ownerPhone": "9876543210" }` → Razorpay order for the joining fee.

### `POST /food/seller/unregistered`
Lead capture for sellers not on the platform: `{ ownerName, sellerName, mobileNumber, emailId, location }`.

### `POST /food/seller/upload-attachment` — multipart, field `file`
...
## 3. Seller profile (Bearer)

### `GET /food/seller/current`
→ `data: { seller }`:
```json
{
  "_id": "…",
  "sellerName": "…", "ownerName": "…", "ownerEmail": "…", "ownerPhone": "…",
  "primaryContactNumber": "…", "pureVegSeller": true,
  "area": "…", "city": "…", "state": "…", "pincode": "…", "landmark": "…",
...
|---|---|---|---|
| PATCH | `/food/seller/profile` | partial seller fields | `{ seller }` |
| PATCH | `/food/seller/availability` | `{ "isAcceptingOrders": true }` | `{ seller }` |
| PATCH | `/food/seller/dining-settings` | `{ "isEnabled": true, "maxGuests": 6, "diningType": "family-dining" }` | `{ seller }` |
| DELETE | *(none — deletion is `deleteCurrentSellerAccount`, not routed publicly)* | | |
...
|---|---|---|---|
| POST | `/food/seller/profile/profile-image` | `file` | 1 |
| POST | `/food/seller/profile/menu-image` | `file` | 1 |
| POST | `/food/seller/profile/cover-images` | `files` | 20 |
| POST | `/food/seller/profile/menu-images` | `files` | 20 |
...
### Public-page banners (the carousel on `/sellers/:id`)

| Method | Path | Body |
|---|---|---|
| GET | `/food/seller/banners` | → `{ banners: [url], primaryBanner, maxBanners: 10 }` |
| POST | `/food/seller/banners` | multipart, field `files` (≤10) → `{ banners, primaryBanner, uploaded, skipped }` |
| DELETE | `/food/seller/banners` | `{ "bannerUrl": "<exact url from the API>" }` |
| PATCH | `/food/seller/banners/order` | `{ "banners": [ ...full list in order... ] }` |
...
Send the **raw relative path** the API returned on delete/reorder, not the absolute URL you built for display, or you'll get *"Banner not found on this seller"*.

⚠️ Do **not** use the older `POST /food/seller/profile/cover-images` for this — it resets the seller to `status: "pending"` and forces admin re-approval. `/banners` never touches status.

### Main cover image + premises gallery

| Method | Path | Body |
|---|---|---|
| GET | `/food/seller/media` | → `{ coverImage, galleryImages, maxGalleryImages: 10 }` |
| POST | `/food/seller/media/cover-image` | multipart, field `file` → `{ coverImage }` |
| POST | `/food/seller/media/gallery` | multipart, field `files` (≤10) → `{ galleryImages, uploaded, skipped }` |
| DELETE | `/food/seller/media/gallery` | `{ "imageUrl": "…" }` |
...
`GET /food/seller/app-banners` (Bearer SELLER, read-only)
```json
{ "banners": [ { "id","imageUrl","title","ctaLink","sortOrder","isActive" } ],
  "recommendedSize": { "width": 350, "height": 100 },
  "aspectRatio": 3.5 }
```
Admin-managed; the seller can't edit these. Size the widget from `aspectRatio`, don't hardcode 350×100. `ctaLink` may be `""` → not tappable. Empty list → render nothing.

---

## 4. Outlet timings

### `GET /food/seller/outlet-timings` (own) · `GET /food/seller/sellers/:id/outlet-timings` (public)
```json
{
  "outletTimings": {
...
### `PUT /food/seller/outlet-timings`
```json
...
Side effect: saving also syncs the seller's top-level `openingTime` / `closingTime` / `openDays` from today's row, and busts the public caches.
...
| Method | Path | Body | Response |
|---|---|---|---|
| GET | `/food/seller/categories` | query `zoneId` (defaults to the seller's zone) | `data` = category list |
| POST | `/food/seller/categories` | category fields | `{ category }` |
| PATCH | `/food/seller/categories/:id` | partial | `{ category }` (404 if missing) |
| DELETE | `/food/seller/categories/:id` | — | delete result |
| GET | `/food/seller/categories/public` | no auth, zone-aware | approved categories |

Unauthenticated calls to the same handler fall through to the public, approved-only list.

### `GET /food/seller/menu` (own) · `GET /food/seller/sellers/:id/menu` (public)
```json
{
  "menu": {
    "sections": [{
...
```
Sections sort by `sortOrder` then name; items inside a section sort newest-first. The public route 404s for a non-approved seller.

### `PATCH /food/seller/menu`
Bulk menu update. → `{ menu }`

### Items

### `POST /food/seller/foods`
```json
{
...
| Method | Path | Notes |
|---|---|---|
| GET | `/food/seller/bulk-upload/template` | returns an **.xlsx file stream**, not JSON — `Content-Disposition: attachment` |
| POST | `/food/seller/bulk-upload` | multipart, field `file`, must be .xlsx |
...
| Method | Path | Body |
|---|---|---|
| GET | `/food/seller/addons` | query: `status` (`pending`/`approved`/`rejected`), `search` (≤80), `page`, `limit` (≤100), `includeDeleted` |
| POST | `/food/seller/addons` | see below |
| PATCH | `/food/seller/addons/:id` | `{ "draft": { …partial… }, "isAvailable": true }` |
| DELETE | `/food/seller/addons/:id` | — |
...
Public read: `GET /food/seller/sellers/:id/addons` → `{ addons }`.
...
Only orders that are actually payable are returned: payment method `cash`/`wallet`, or payment status in `paid`/`authorized`/`captured`/`settled`/`refunded`. Unpaid `pending_payment` orders never reach the seller.
...
### `GET /food/seller/orders/:orderId` → `{ order }`

### `PATCH /food/seller/orders/:orderId/status`
```json
{ "orderStatus": "preparing", "note": "optional" }
```
Allowed values: `confirmed`, `preparing`, `ready_for_pickup`, `picked_up`, `delivered`, `cancelled_by_seller`. In practice the seller drives `confirmed` → `preparing` → `ready_for_pickup`; the rider owns the rest.
→ `{ order }`

### `POST /food/seller/orders/:orderId/resend-notification`
Re-pings delivery partners for an order that hasn't been picked up.

### Order object

Same canonical shape as the user app, with `userId` populated. Key fields for the seller screen:
```json
{
  ...
  "statusHistory": [{ "at": "…", "byRole": "SELLER", "byId": "…", "from": "confirmed", "to": "preparing", "note": "" }],
  "ratings": { "seller": { "rating": 5, "comment": "…", "ratedAt": "…" },
               "deliveryPartner": { … } },
  "rating": 5,
  "note": "kitchen note",
...
Full status enum on the order document: `pending_payment`, `created`, `confirmed`, `preparing`, `ready_for_pickup`, `reached_pickup`, `picked_up`, `reached_drop`, `delivered`, `cancelled_by_user`, `cancelled_by_seller`, `cancelled_by_admin`.

---

## 7. Finance & payouts

### `GET /food/seller/finance`
Query: `page`, `limit`, `startDate`, `endDate` (the date range drives `pastCycles` only).
```json
{
  "seller": {
    "name": "…", "sellerId": "REST000123", "address": "…",
    "subscriptionDueAmount": 2360, "subscriptionStatus": "due"
  },
  "subscription": { "lockedAmount": 2360, "lockedMonths": "May, Jun 2026", "openInvoices": 2 },
  "features": { "sellerSubscriptionEnabled": true },
  "wallet": {
    "totalEarnings": 84200,
    ...
    "orders": [{
      ...
      "discount": 50, "adminDiscountShare": 25, "sellerDiscountShare": 25,
      "discountAdminBearPercentage": 50, "discountSellerBearPercentage": 50,
...
### `GET /food/seller/subscription/overview`
```json
{
  "featureEnabled": true,
  "currentMonth": {
...
### `GET /food/seller/subscription/invoices`
...
### `POST /food/seller/my-offers`
`sellerScope` and `sellerId` are injected server-side — don't send them.
```json
{
  "couponCode": "SAVE50",
...
```
→ 201, `{ doc }`. A seller-created offer is always `sellerBearPercentage: 100` / `adminBearPercentage: 0` — the seller funds its own discounts. An `endDate` in the past creates the offer as `inactive`.

Duplicate `couponCode` across the platform is rejected.

| Method | Path | Body | Response |
|---|---|---|---|
| GET | `/food/seller/my-offers` | — | `{ offers: [...] }` |
| PATCH | `/food/seller/my-offers/:id/status` | `{ "status": "active" }` | `{ doc }` |
| DELETE | `/food/seller/my-offers/:id` | — | no `data` |

Public read (user app): `GET /food/seller/offers` — optional auth, personalized when a token is sent.
...
### `GET /food/seller/complaints`
Query passes through to the admin complaint service (pagination + filters). → `data` = complaint list.

### `POST /food/seller/support/tickets`
```json
{
  "category": "orders",        // orders | payments | menu | seller | technical | other — required
...
### `GET /food/seller/support/tickets` → `{ tickets: [...] }` (+ pagination)

### `POST /food/seller/feedback-experience`
...
Handshake with the access token. On connect the server auto-joins `seller:<sellerId>`.

**Emit:**
| Event | Payload | Notes |
|---|---|---|
| `join-seller` | `sellerId` | explicit re-join; ack `seller-room-joined`. Only your own id is accepted |

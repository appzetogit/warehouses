...
import { CashbackSettings } from '../models/cashbackSettings.model.js';
...
export async function upsertCashbackSettingsController(req, res, next) {
    try {
        ...
        const existing = await CashbackSettings.findOne({ isActive: true }).sort({ createdAt: -1 });
        const cashbackSettings = existing
            ? await CashbackSettings.findByIdAndUpdate(existing._id, { $set }, { new: true }).lean()
            : (await CashbackSettings.create({ ...$set, isActive: true })).toObject();

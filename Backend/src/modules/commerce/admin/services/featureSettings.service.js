import { FeatureSetting } from '../models/featureSetting.model.js';
...
export async function ensureDefaultFeatureSettings() {
    for (const feature of DEFAULT_FEATURES) {
        await FeatureSetting.updateOne(
            { key: feature.key },
            { $setOnInsert: feature },
            { upsert: true }
        );
...
export async function listFeatureSettings() {
    ...
    const docs = await FeatureSetting.find({}).sort({ createdAt: 1 }).lean();
    return docs.map((doc) => ({
        key: doc.key,
        name: doc.name,
...
export async function updateFeatureSetting(key, payload = {}) {
    ...
    const updated = await FeatureSetting.findOneAndUpdate(
        { key: String(key || '').trim() },
...
export async function isFeatureEnabled(key, fallback = true) {
    ...
    await ensureDefaultFeatureSettings();
    const doc = await FeatureSetting.findOne({ key: String(key).trim() }).select('isEnabled').lean();

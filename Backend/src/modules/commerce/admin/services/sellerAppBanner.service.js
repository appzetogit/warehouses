...
export const listBannersAdmin = async () => {
    const docs = await SellerAppBanner.find({})
...
export const listBannersForSellerApp = async () => {
    const docs = await SellerAppBanner.find({ isActive: true })
        .sort({ sortOrder: 1, createdAt: -1 })
        .lean();
    return {
        banners: docs.map(serialize),
...
export const createBanner = async (file, body = {}) => {
    ...
    const last = await SellerAppBanner.findOne({}).sort({ sortOrder: -1 }).select('sortOrder').lean();
    const sortOrder = body.sortOrder !== undefined
        ? Number(body.sortOrder) || 0
        : (Number(last?.sortOrder) || 0) + 1;

    const doc = await SellerAppBanner.create({
        imageUrl,
...
export const updateBanner = async (id, body = {}, file = null) => {
    assertId(id);
    const doc = await SellerAppBanner.findById(id);
...
export const deleteBanner = async (id) => {
    assertId(id);
    const res = await SellerAppBanner.deleteOne({ _id: id });
...
export const toggleBannerStatus = async (id, isActive) => {
    assertId(id);
    const doc = await SellerAppBanner.findByIdAndUpdate(
        id,
...
export const reorderBanners = async (ids) => {
    ...
    await Promise.all(
        list.map((id, index) =>
            SellerAppBanner.updateOne({ _id: id }, { $set: { sortOrder: index } })
        )
    );

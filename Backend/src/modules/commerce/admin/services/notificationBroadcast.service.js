...
const buildSellerLabel = (doc) => ({
    label: String(doc?.sellerName || doc?.ownerName || 'Seller').trim(),
...
const modelConfigMap = {
    ...
    SELLER: {
        ...
        select: '_id sellerName ownerName ownerPhone ownerEmail',
...
const resolveCustomTargets = async ({ targets = [], targetIds = [] } = {}) => {
    ...
    const users = await User.find({ _id: { $in: ids }, isActive: true }).select('_id name phone email').lean();
...
export const deleteBroadcastNotification = async (broadcastId) => {
    ...
    const result = await Notification.deleteMany({ broadcastId: normalizedId });

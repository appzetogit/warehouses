...
export async function syncRazorpayQrPayment(orderDoc) {
  ...
  // OrderTransaction is source of truth; Order.payment is fallback
  const tx = await OrderTransaction.findOne({ orderId }).lean();
  ...
  // Allow sync if either tx OR Order has razorpay_qr method
  ...
  // Update OrderTransaction (upsert in case it didn't exist)
  await OrderTransaction.updateOne(
    { orderId },
    {
      $set: {
  ...
  const updatedTx = await OrderTransaction.findOne({ orderId }).lean();
...
export async function createCollectQr(
  ...
  // CRITICAL: use upsert so this works even if OrderTransaction was never created at order placement
  const upsertData = {
    $set: {
  ...
  // Also write to Order so sync can find paymentLinkId even without a TX doc
  await Order.updateOne(
    { _id: order._id },
    {
      $set: {
  ...
  const updatedTx = await OrderTransaction.findOne({ orderId: order._id }).lean();


  if (updatedTx) {
    await orderTransactionService.updateTransactionStatus(
      order._id,
...
export async function getPaymentStatus(orderId, deliveryPartnerId) {
  ...
  const order = await Order.findOne(identity).select(
  ...
  let transaction = await OrderTransaction.findOne({ orderId: order._id }).lean();
  ...
  if (effectiveMethod === 'razorpay_qr' && transaction?.payment?.status !== 'paid' && hasPaymentLink) {
    ...
    transaction = await OrderTransaction.findOne({ orderId: order._id }).lean();
    logger.info(`[getPaymentStatus] After sync: tx.payment.status=${transaction?.payment?.status}`);
  }

  // If no transaction, fall back to Order.payment
...
export async function switchToCash(orderId, deliveryPartnerId) {
  ...
  if (order.payment?.qr?.paymentLinkId) {
    try {
      await syncRazorpayQrPayment(await Order.findOne(query));
  ...
  // Order.payment as razorpay_qr/pending_qr, so completeDelivery never flipped it to
  // paid and the cash-in-hand aggregations (which filter on Order.payment.method
  // 'cash' + status 'paid') never saw the money the rider actually collected.
  await Promise.all([
    OrderTransaction.updateOne(
      { orderId: order._id },
      {
        $set: {
          paymentMethod: 'cash',

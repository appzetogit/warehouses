...
import { DeliveryCashLimit } from '../../admin/models/deliveryCashLimit.model.js';
import {
  ValidationError,
  ForbiddenError,
  NotFoundError,
...
export async function getCurrentTripDelivery(deliveryPartnerId) {
  ...
  const tx = await OrderTransaction.findOne({ orderId: order._id })
...
async function assertCashLimitAllows(deliveryPartnerId, order) {
  ...
  const [settings, wallet] = await Promise.all([
    DeliveryCashLimit.findOne({ isActive: true }).select('deliveryCashLimit').lean(),
...
export async function acceptOrderDelivery(orderId, deliveryPartnerId) {
  ...
  if (alreadyOnTrip) {
    ...
    const requestedOrder = await Order.findOne(identity).select('_id').lean();
    const requestedOrderKey = String(requestedOrder?._id || '');

    if (activeOrderKey && requestedOrderKey && activeOrderKey === requestedOrderKey) {
      const acceptedOrder = await Order.findOne(identity).populate('sellerId userId');
  ...
  {
    const pending = await Order.findOne(identity)
  ...
  if (!order) {
    const existing = await Order.findOne(identity)
    ...
    if (
      ...
      const acceptedOrder = await Order.findOne(identity)
        .populate('sellerId userId');
  ...
  void (async () => {
    ...
    try {
      await orderTransactionService.updateTransactionRider(order._id, deliveryPartnerId);
    } catch (error) {
      logger.error(
...
export async function completeDelivery(orderId, deliveryPartnerId, body = {}) {
  ...
  if (
    ...
    const orderWithSecret = await Order.findById(order._id).select('+deliveryOtp');

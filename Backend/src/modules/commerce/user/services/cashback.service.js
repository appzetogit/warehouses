...
import { CashbackSettings } from '../../admin/models/cashbackSettings.model.js';
import { Order } from '../../orders/models/order.model.js';
import { logger } from '../../../../utils/logger.js';

export const getActiveCashbackSettings = async () => {
    const doc = await CashbackSettings.findOne({ isActive: true })
        .sort({ createdAt: -1 })
        .lean();
    return (
        doc || {
...
export const awardOrderCashback = async (orderId) => {
    try {
        ...
        const wallet = await UserWallet.findOne({ userId: userOid });
...
export const getCashbackHistory = async (userId, query = {}) => {
    ...
    const wallet = await UserWallet.findOne({ userId: new mongoose.Types.ObjectId(id) })
        .select('transactions')
        .lean();
...
export const getUserRefundHistory = async (userId, query = {}) => {
    ...
    const [docs, total, wallet] = await Promise.all([
        ...
        UserWallet.findOne({ userId: oid }).select('transactions').lean()
    ]);

...
export async function sendMessage(sender, dto) {
    ...
    if (orderIdRaw && !adminInvolved) {
        ...
        const order = await Order.findById(orderIdRaw)
            .select('userId sellerId dispatch.deliveryPartnerId')
            .lean();
        if (!order) throw new ValidationError('Order not found');
...
export async function getHistory(me, { conversationId, page = 1, limit = 30 }) {
    ...
    const first = await ChatMessage.findOne({ conversationId }).select('participants').lean();
    ...
    const [docs, total] = await Promise.all([
        ChatMessage.find({ conversationId }).sort({ createdAt: -1 }).skip((p - 1) * l).limit(l).lean(),

...
export const getSubscriptionOverviewController = async (req, res, next) => {
    try {
        ...
        const featureEnabled = await isFeatureEnabled(FEATURE_KEYS.SELLER_SUBSCRIPTION, true);
        ...
        const [gmvResult, settings, outstanding, finance] = await Promise.all([
            computeMonthlyGmv(sellerId, start, new Date()),
            getSellerSubscriptionSettings(),
            getOutstandingSummary(sellerId),
            getSellerFinance(sellerId),
        ]);
...
/**
 * GET /subscription/invoices — the seller's monthly invoice history.
 */
...
export const getSubscriptionInvoiceController = async (req, res, next) => {
    try {
        ...
        const transactions = await SubscriptionTransaction.find({ invoiceId: invoice._id })
            .sort({ createdAt: 1 })
            .lean();
...
/**
 * GET /subscription/transactions — complete billing timeline for the seller.
 */

...
import { SellerSupportTicket } from '../models/supportTicket.model.js';
...
export const createSellerSupportTicketController = async (req, res, next) => {
    try {
        ...
        const created = await SellerSupportTicket.create({
...
export const listSellerSupportTicketsController = async (req, res, next) => {
    try {
        ...
        const [tickets, total] = await Promise.all([
            SellerSupportTicket.find(filter)
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
            SellerSupportTicket.countDocuments(filter)
        ]);

...
export const getDeliveryReferralStats = async (deliveryPartnerId) => {
    ...
    const [partner, settingsDoc, bonusAgg, logs] = await Promise.all([
        DeliveryPartner.findById(oid).select('_id referralCount referralCode').lean(),
        ReferralSettings.findOne({ isActive: true }).sort({ createdAt: -1 }).lean(),
        DeliveryBonusTransaction.aggregate([
            { $match: { deliveryPartnerId: oid, reference: { $regex: /referral/i } } },
            { $group: { _id: null, total: { $sum: '$amount' } } }
        ]),
        ReferralLog.find({ referrerId: oid, role: 'DELIVERY_PARTNER' })
            .sort({ createdAt: -1 })
            .lean()
    ]);
    ...
    const invitedRaw = await DeliveryPartner.find({ referredBy: oid })
 ...
 *
 * Idempotent: ReferralLog has a unique index on { refereeId, role }, so at most one
...
export const creditDeliveryReferralOnFirstDelivery = async (refereePartnerId) => {
    try {
        ...
        const referee = await DeliveryPartner.findById(id)
        ...
        const existing = await ReferralLog.findOne({
        ...
        const [settingsDoc, referrer] = await Promise.all([
            ReferralSettings.findOne({ isActive: true }).sort({ createdAt: -1 }).lean(),
            DeliveryPartner.findById(referee.referredBy)
        ...
        await Promise.all([
            DeliveryPartner.updateOne({ _id: referrer._id }, { $inc: { referralCount: 1 } }),

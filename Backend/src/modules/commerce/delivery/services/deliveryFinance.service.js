...
import { DeliveryCashDeposit } from '../models/deliveryCashDeposit.model.js';
...
export const getDeliveryPartnerWalletEnhanced = async (deliveryPartnerId) => {
    ...
    const [cashLimitSettings, earningsAgg, cashCollectedAgg, cashDepositsAgg, bonusAgg, withdrawalAgg, withdrawalsList, depositList, walletDoc] = await Promise.all([
        ...
        DeliveryWithdrawal.find({ deliveryPartnerId: partnerId })
            .sort({ createdAt: -1 })
            .limit(50)
            .lean(),
        DeliveryCashDeposit.find({ deliveryPartnerId: partnerId })
            .sort({ createdAt: -1 })
            .limit(50)
            .lean(),
        DeliveryWallet.findOne({ deliveryPartnerId: partnerId }).lean()
    ]);
    ...
    const [ordersTx] = await Promise.all([
        Order.find({ 'dispatch.deliveryPartnerId': partnerId, orderStatus: 'delivered' })
            .sort({ createdAt: -1 })
...
export const requestDeliveryWithdrawal = async (deliveryPartnerId, payload) => {
    ...
    const [partner, walletDoc, pendingAgg] = await Promise.all([
        DeliveryPartner.findById(deliveryPartnerId).lean(),
        DeliveryWallet.findOne({ deliveryPartnerId: partnerId }),
        DeliveryWithdrawal.aggregate([
            {
                $match: {
                    deliveryPartnerId: partnerId,

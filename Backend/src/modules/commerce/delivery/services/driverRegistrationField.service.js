...
import { DriverRegistrationField } from '../models/driverRegistrationField.model.js';
...
export async function listFields() {
    const docs = await DriverRegistrationField.find({})
...
export async function createField(body = {}) {
    ...
    const exists = await DriverRegistrationField.findOne({ key }).lean();
    if (exists) throw new ValidationError('A field with this key already exists');

    const doc = await DriverRegistrationField.create({
        key,
        label,
        type,
...
export async function updateField(id, body = {}) {
    ...
    const doc = await DriverRegistrationField.findById(id);
...
export async function deleteField(id) {
    if (!mongoose.Types.ObjectId.isValid(String(id))) throw new ValidationError('Invalid field id');
    const doc = await DriverRegistrationField.findById(id).lean();
    if (!doc) throw new ValidationError('Field not found');
    if (doc.isSystem) throw new ValidationError('System fields cannot be deleted; deactivate instead');
    await DriverRegistrationField.deleteOne({ _id: id });
...
export async function getPublicFormSchema() {
    const docs = await DriverRegistrationField.find({ isActive: true })
        .sort({ page: 1, order: 1, createdAt: 1 })
        .lean();
...
export async function collectDynamicRegistration(body = {}, fileKeys = new Set()) {
    const fields = await DriverRegistrationField.find({ isActive: true }).lean();
    const customFields = {};

...
import { EarningAddon } from '../../admin/models/earningAddon.model.js';
...
export const registerDeliveryPartner = async (payload, files, rawBody = {}) => {
    ...
    if (existing) {
        ...
        await DeliveryPartner.deleteMany({ phone });
    }
    ...
    if (vehicleNumber && String(vehicleNumber).trim()) {
        ...
        const activeVehicle = await DeliveryPartner.findOne({ 
            vehicleNumber: vNum, 
            status: { $ne: 'rejected' } 
        });
        if (activeVehicle) {
    ...
    if (refRaw && mongoose.Types.ObjectId.isValid(refRaw) && String(refRaw) !== String(partner._id)) {
        const referrer = await DeliveryPartner.findById(refRaw).select('_id').lean();
        if (referrer) {
...
export const getDeliveryPocketDetails = async (deliveryPartnerId, query = {}) => {
    ...
    const paymentTransactions = (orders || []).map((o) => ({
        ...
        orderId: o.orderId || String(o._id),
        metadata: { orderId: o.orderId || String(o._id) },
        description: o?.sellerId?.sellerName ? `Order earning - ${o.sellerId.sellerName}` : 'Order earning'
    }));
...
export const getActiveEarningAddonsForPartner = async (deliveryPartnerId) => {
    ...
    const addons = await EarningAddon.find({
        status: 'active',
        startDate: { $lte: now },
        endDate: { $gte: now }
    })
...
export const deleteDeliveryPartnerAccount = async (partnerId) => {
    ...
    const { DeliveryWallet } = await import('../models/deliveryWallet.model.js');

    const partner = await DeliveryPartner.findById(partnerId);
    if (!partner) throw new ValidationError('Delivery partner not found');

    // Remove associated documents
    await DeliveryWallet.findOneAndDelete({ deliveryPartnerId: partnerId });

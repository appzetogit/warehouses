...
async function deassignOrderForRedispatch({
    ...
    const existingOrder = await Order.findOne(identity).lean();
...
export async function listOrderEmergencyRequestsAdmin(query = {}) {
    ...
    if (query.search && String(query.search).trim()) {
        ...
        const partnerIds = await DeliveryPartner
            .find({
                $or: [
...
export async function deassignAndResendEmergencyOrder(requestId, adminId) {
    ...
    let order = null;
    try {
        const existingOrder = await Order.findById(request.orderId).lean();
        if (!existingOrder) throw new NotFoundError('Order not found');

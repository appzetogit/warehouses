...
const getNextSortOrder = async () => {
    const last = await ExploreIcon.findOne().sort({ sortOrder: -1 }).select('sortOrder').lean();
...
export const createExploreIcon = async (file, meta) => {
    ...
    const doc = await ExploreIcon.create({
        label,
        iconUrl: saved.url,
        publicId: saved.path,
        linkType: 'custom',
...
export const updateExploreIcon = async (id, payload) => {
    const doc = await ExploreIcon.findById(id);
    if (!doc) {
        return null;
    }
    ...
    const updated = await ExploreIcon.findByIdAndUpdate(id, updates, { new: true }).lean();
    return updated;
};
...
export const toggleExploreIconStatus = async (id) => {
    ...
    const updated = await ExploreIcon.findByIdAndUpdate(id, { isActive }, { new: true }).lean();
    return updated;
};
...
export const updateExploreIconOrder = async (id, order) => {
    ...
    const updated = await ExploreIcon.findByIdAndUpdate(id, { sortOrder: num }, { new: true }).lean();
    return updated;
};

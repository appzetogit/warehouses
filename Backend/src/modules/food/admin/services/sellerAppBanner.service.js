...
import {
    FoodSellerAppBanner,
    SELLER_APP_BANNER_SIZE
} from '../models/sellerAppBanner.model.js';
...
export const listBannersAdmin = async () => {
    const docs = await FoodSellerAppBanner.find({})
        .sort({ sortOrder: 1, createdAt: -1 })
        .lean();
    return {
        banners: docs.map(serialize),
        recommendedSize: SELLER_APP_BANNER_SIZE
    };
};

/** Seller app: active banners only, in display order. */
export const listBannersForSellerApp = async () => {
    const docs = await FoodSellerAppBanner.find({ isActive: true })
        .sort({ sortOrder: 1, createdAt: -1 })
        .lean();
    return {
        banners: docs.map(serialize),
        recommendedSize: SELLER_APP_BANNER_SIZE,
        aspectRatio: Number(
            (SELLER_APP_BANNER_SIZE.width / SELLER_APP_BANNER_SIZE.height).toFixed(2)
        )
    };
};
...
export const createBanner = async (file, body = {}) => {
    ...
    const last = await FoodSellerAppBanner.findOne({}).sort({ sortOrder: -1 }).select('sortOrder').lean();
    const sortOrder = body.sortOrder !== undefined
        ? Number(body.sortOrder) || 0
        : (Number(last?.sortOrder) || 0) + 1;

    const doc = await FoodSellerAppBanner.create({
        imageUrl,
...
export const updateBanner = async (id, body = {}, file = null) => {
    assertId(id);
    const doc = await FoodSellerAppBanner.findById(id);
...
export const deleteBanner = async (id) => {
    assertId(id);
    const res = await FoodSellerAppBanner.deleteOne({ _id: id });
...
export const toggleBannerStatus = async (id, isActive) => {
    assertId(id);
    const doc = await FoodSellerAppBanner.findByIdAndUpdate(
        id,
...
export const reorderBanners = async (ids) => {
    ...
    await Promise.all(
        list.map((id, index) =>
            FoodSellerAppBanner.updateOne({ _id: id }, { $set: { sortOrder: index } })
        )
    );

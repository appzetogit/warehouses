...
const sellerSchema = new mongoose.Schema(
  {
    ...
    // These are derived from sellerName/ownerPhone at write time.
     ...
     * Manual availability override controlled by seller dashboard Online/Offline toggle.
    ...
    /**
     * Legacy field. No longer used to keep a seller online outside outlet hours.
    ...
    /** Single hero/cover image for the seller page. Falls back to coverImages[0]. */
    coverImage: {
      type: String,
      default: '',
    },
...
sellerSchema.pre("validate", function normalizeDerivedFields(next) {
  const name =
    typeof this.sellerName === "string" ? this.sellerName : "";
  const normalizedName = name.trim().toLowerCase().replace(/\s+/g, " ");
  this.sellerNameNormalized = normalizedName || undefined;
...
sellerSchema.index({ ownerPhone: 1 });
sellerSchema.index({ sellerName: 1 });
sellerSchema.index({ sellerNameNormalized: 1 });
sellerSchema.index({ city: 1 });
sellerSchema.index({ "location.city": 1 });
sellerSchema.index({ location: "2dsphere" });
sellerSchema.index({ sellerName: 1, ownerPhone: 1 });
...
sellerSchema.index(
  { sellerNameNormalized: 1, ownerPhoneLast10: 1 },
  {
    unique: true,
    partialFilterExpression: {
      sellerNameNormalized: { $type: "string" },
...
export const FoodSeller = mongoose.model(
  "FoodSeller",
  sellerSchema,
);

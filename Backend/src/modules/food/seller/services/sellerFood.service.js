 ...
 */
export async function updateSellerFoodStock(sellerId, entries = []) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            const doc = await FoodItem.findOneAndUpdate(
                { _id: new mongoose.Types.ObjectId(foodId), sellerId: context.sellerId },
                {
...
export async function listLowStockFoods(sellerId) {
    const context = await getSellerContext(sellerId);
...
export async function createSellerFood(sellerId, body = {}) {
    const context = await getSellerContext(sellerId);
...
export async function updateSellerFood(sellerId, foodId, body = {}) {
    const context = await getSellerContext(sellerId);
    if (!foodId || !mongoose.Types.ObjectId.isValid(String(foodId))) {
        throw new ValidationError('Invalid food id');
    }

    const existing = await FoodItem.findOne({ _id: foodId, sellerId }).lean();
 ...
 */
export async function deleteSellerFood(sellerId, foodId) {
    const context = await getSellerContext(sellerId);

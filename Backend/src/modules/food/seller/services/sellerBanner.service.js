...
const loadSeller = async (sellerId) => {
    ...
    const doc = await FoodSeller.findById(sellerId).select('coverImages profileImage').lean();
...
export const uploadSellerCoverImage = async (sellerId, file) => {
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, { $set: { coverImage: url } });
    bustPublicCaches();
...
export const uploadSellerGalleryImages = async (sellerId, files = []) => {
    ...
    const doc = await FoodSeller.findById(sellerId).select('galleryImages').lean();
    if (!doc) throw new ValidationError('Store not found');
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, {
...
export const deleteSellerGalleryImage = async (sellerId, imageUrl) => {
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, { $set: { galleryImages } });
    bustPublicCaches();
...
export const uploadSellerBanners = async (sellerId, files = []) => {
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, { $set: update });
    bustPublicCaches();
...
export const deleteSellerBanner = async (sellerId, bannerUrl) => {
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, { $set: { coverImages: banners } });
    bustPublicCaches();
...
export const reorderSellerBanners = async (sellerId, orderedUrls) => {
    ...
    await FoodSeller.findByIdAndUpdate(sellerId, { $set: { coverImages: next } });
    bustPublicCaches();

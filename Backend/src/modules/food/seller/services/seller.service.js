...
export const registerSeller = async (payload, files) => {
     ...
     */
    const existing = await FoodSeller.findOne({
        $or: [
            { ownerPhoneLast10 },
            { ownerPhone },
    ...
    try {
        ...
        const seller = await FoodSeller.create({
            sellerName,
...
export const listApprovedSellers = async (query = {}) => {
    ...
    if (lat !== null && lng !== null && wantsGeo) {
        ...
        const [pageDocs, totalDocs] = await Promise.all([
            FoodSeller.aggregate([
                ...basePipeline,
                { $project: projection },
                { $skip: skip },
                { $limit: limit }
            ]),
            FoodSeller.aggregate([...basePipeline, { $count: 'count' }])
        ]);
...
export const getApprovedSellerByIdOrSlug = async (idOrSlug) => {
    ...
    if (/^[0-9a-fA-F]{24}$/.test(value)) {
        const doc = await FoodSeller.findOne({ _id: value, status: 'approved' })
    ...
    const doc = await FoodSeller.findOne({
        status: 'approved',
        sellerNameNormalized
    })
        .select(PUBLIC_SELLER_SELECT)

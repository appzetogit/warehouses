...
export async function getSellerMenu(sellerId) {
    ...
    await restoreExpiredFoodAvailability({ sellerId });
    const foods = await FoodItem.find({ sellerId })
        .sort({ createdAt: -1 })
        .limit(5000)
        .lean();
...
export async function getPublicApprovedSellerMenu(sellerIdOrSlug) {
    ...
    if (/^[0-9a-fA-F]{24}$/.test(value)) {
        seller = await FoodSeller.findOne({ _id: value, status: 'approved' })
            .select('_id status')
            .lean();
    } else {
        const normalized = value.trim().toLowerCase().replace(/-/g, ' ').replace(/\s+/g, ' ');
        seller = await FoodSeller.findOne({ sellerNameNormalized: normalized, status: 'approved' })
            .select('_id status')
            .lean();
    }
    ...
    await restoreExpiredFoodAvailability({ sellerId: seller._id });
    const foods = await FoodItem.find({ sellerId: seller._id, approvalStatus: 'approved' })
        .sort({ createdAt: -1 })
        .limit(2000)
        .lean();
    return buildMenuFromFoods(foods);

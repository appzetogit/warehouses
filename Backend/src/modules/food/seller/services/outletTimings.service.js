...
export async function getOutletTimingsForSeller(sellerId) {
    ...
    const doc = await FoodSellerOutletTimings.findOne({ sellerId }).select('timings updatedAt').lean();
...
export async function upsertOutletTimingsForSeller(sellerId, outletTimings) {
    ...
    const doc = await FoodSellerOutletTimings.findOneAndUpdate(
        { sellerId },
        { $set: { timings } },
...
export async function getOutletTimingsMapForSellers(sellerIds = [], options = {}) {
    ...
    const docs = await FoodSellerOutletTimings.find({ sellerId: { $in: objectIds } })

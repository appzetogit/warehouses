 ...
 *
 * `GET /sellers/:id/addons` is cached for 600s and NOTHING was clearing it, so a
...
export async function invalidatePublicAddonCache() {
    try {
        ...
        await invalidateCache('seller_addons:*');
    } catch (err) {
...
/**
 * Keeps only ids that are real menu items of THIS seller.
 *
 * Without this check a seller could attach its add-ons to another seller's
 * menu items, and those add-ons would then surface on that seller's item sheet
 * at a price its owner never set.
 */
async function sanitizeFoodIds(sellerId, foodIds) {
...
const normalizeAddonDoc = (doc) => {
    ...
    return {
        ...
        // Draft fields (what seller edits)
        name: draft.name || '',
...
export async function listSellerAddons(sellerId, query = {}) {
...
export async function createSellerAddon(sellerId, body) {
    if (!sellerId || !mongoose.Types.ObjectId.isValid(String(sellerId))) {
        throw new ValidationError('Invalid store id');
    }
    ...
    // Prevent duplicates per seller among non-deleted docs (case-insensitive exact).
    ...
    try {
        ...
        void notifyAdminsSafely({
            ...
            body: `Seller has submitted a new addon "${name}" for approval.`,
            data: {
...
export async function updateSellerAddon(sellerId, addonId, updateDto) {
    if (!sellerId || !mongoose.Types.ObjectId.isValid(String(sellerId))) {
        throw new ValidationError('Invalid store id');
    }
    ...
    if (Object.keys(set).length === 0) {
        const existing = await FoodAddon.findOne({ _id, sellerId: rid, isDeleted: { $ne: true } }).lean();
        return existing ? normalizeAddonDoc(existing) : null;
    }

    const updated = await FoodAddon.findOneAndUpdate(
        { _id, sellerId: rid, isDeleted: { $ne: true } },
        { $set: set },
        { new: true }
    ).lean();
...
export async function deleteSellerAddon(sellerId, addonId) {
    if (!sellerId || !mongoose.Types.ObjectId.isValid(String(sellerId))) {
        throw new ValidationError('Invalid store id');
    }
    if (!addonId || !mongoose.Types.ObjectId.isValid(String(addonId))) {

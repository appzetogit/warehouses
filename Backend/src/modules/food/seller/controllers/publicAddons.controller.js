...
import { getPublicApprovedSellerAddons } from '../services/publicAddons.service.js';

export const getPublicSellerAddonsController = async (req, res, next) => {
    try {
        // ?foodId=<menu item id> narrows to that item's add-ons plus any
        // whole-menu ones. Omit it to get everything the seller offers.
        const result = await getPublicApprovedSellerAddons(req.params.id, {

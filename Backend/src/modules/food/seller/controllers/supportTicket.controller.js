...
import { FoodSellerSupportTicket } from '../models/supportTicket.model.js';
import { sendError, sendResponse } from '../../../../utils/response.js';

const ALLOWED_CATEGORIES = ['orders', 'payments', 'menu', 'seller', 'technical', 'other'];
...
export const createSellerSupportTicketController = async (req, res, next) => {
    try {
        ...
        const created = await FoodSellerSupportTicket.create({
...
export const listSellerSupportTicketsController = async (req, res, next) => {
    try {
        ...
        const [tickets, total] = await Promise.all([
            FoodSellerSupportTicket.find(filter)
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
            FoodSellerSupportTicket.countDocuments(filter)
        ]);

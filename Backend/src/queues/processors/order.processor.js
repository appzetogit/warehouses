...
export const processOrderJob = async (job) => {
    ...
    if (action === 'DISPATCH_TIMEOUT_CHECK') {
        try {
            const { processDispatchTimeout } = await import('../../modules/commerce/orders/services/order.service.js');
    ...
    if (action === 'ORDER_ACCEPTANCE_TIMEOUT_CHECK') {
        try {
            const { expireUnacceptedOrderById } = await import('../../modules/commerce/orders/services/order.service.js');

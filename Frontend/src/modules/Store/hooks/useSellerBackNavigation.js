...
const toSellerPath = (value) => {
  ...
  if (trimmed === "/seller") return "/seller"
  if (trimmed.startsWith("/seller/")) return `/food${trimmed}`

  return null
}

const getNormalizedSellerPath = (pathname) => {
...
const resolveSellerBackPath = ({ pathname, state }) => {
  const normalizedPath = getNormalizedSellerPath(pathname)
  const explicitBackPath = toSellerPath(state?.backTo) || toSellerPath(state?.from)
...
export default function useSellerBackNavigation() {
  const navigate = useNavigate()
  const location = useLocation()

  return useCallback(() => {
    navigate(resolveSellerBackPath(location))
  }, [location, navigate])
}

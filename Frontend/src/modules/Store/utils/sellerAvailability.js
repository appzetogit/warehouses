...
export const getSellerAvailabilityStatus = (seller, now = new Date(), options = {}) => {
  if (!seller) {
    return {
      isOpen: false,
      isActive: false,
  ...
  const checkDayWindow = (targetDate) => {
    ...
    const openingTime = timing?.openingTime || seller?.deliveryTimings?.openingTime || seller?.openingTime || null
    const closingTime = timing?.closingTime || seller?.deliveryTimings?.closingTime || seller?.closingTime || null

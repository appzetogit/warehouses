...
export function DualMoney({
  ...
  offClassName = "inline-flex items-center rounded-full bg-wh-brand px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-wh-text tabular-nums",
...
export default function ProductPriceDisplay({
  ...
  const price = Math.round(
    priceProp != null ? Number(priceProp) || 0 : getProductDisplayPrice(item),
  )
  ...
  const showStarting =
    startingFrom != null ? Boolean(startingFrom) : hasProductVariants(item || {})
  const showStrike = hasProductStrikePrice(item, price, otherPrice)
  const discountPercent = showStrike
    ? getProductDiscountPercent(item, price, otherPrice)
    : 0

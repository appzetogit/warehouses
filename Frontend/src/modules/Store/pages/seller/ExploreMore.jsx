...
export default function ExploreMore() {
  ...
  const STORAGE_KEY = "seller_schedule_off"
  ...
  useEffect(() => {
    ...
    const fetchWallet = async () => {
      try {
        const response = await sellerAPI.getWallet()
  ...
  // Get user data from logged in session and seller data
  const userData = useMemo(() => {
    const sessionUser = getCurrentUser("seller")
    
    // Priority 1: Data from the currently logged in session user
    if (sessionUser && sessionUser.name && sessionUser.role) {
      return {
        name: sessionUser.name,
        phone: sessionUser.phone || sellerData?.ownerPhone || sellerData?.phone || "N/A",
        email: sessionUser.email || sellerData?.ownerEmail || sellerData?.email || "N/A",
        role: sessionUser.role.toUpperCase(),
        profileImage: sessionUser.profileImage || sellerData?.profileImage
      }
    }
    
    // Priority 2: Data from the seller document owner fields
    if (sellerData) {
      return {
        name: sellerData.ownerName || sellerData.name || "Seller Owner",
        phone: sellerData.ownerPhone || sellerData.phone || "N/A",
        email: sellerData.ownerEmail || sellerData.email || "N/A",
        role: "OWNER",
    ...
    return {
      name: loadingSeller ? "Loading..." : "Seller Owner",
      phone: "",
      email: "",
      role: "OWNER"
    }
  }, [sellerData, loadingSeller])

  // Get seller display data
  const sellerDisplayName = sellerData?.name || "Loading..."
  const sellerDisplayAddress = sellerData?.location ? formatAddress(sellerData.location) : ""
  ...
  const handleLogout = async () => {
    ...
    try {
      await logoutSellerSession({ navigate })
    } catch (error) {
  ...
  const scheduleOffReasons = [
    "renovation or relocation of seller",
          ...
          return (
            ...
            >
              <motion.button
                ...
                className="group relative flex w-full flex-col items-center justify-center gap-2 overflow-hidden rounded-[20px] bg-white p-4 shadow-[0_2px_10px_rgb(0,0,0,0.02)] ring-1 ring-black/[0.04] transition-all duration-300 hover:shadow-[0_8px_30px_rgb(253,146,11,0.06)] hover:ring-wh-brand/10 hover:bg-wh-brand-600/[0.02] min-h-[105px] md:min-h-[120px] md:rounded-2xl md:p-5 md:shadow-sm md:ring-slate-200/80 md:hover:shadow-md"
              >
                <div className="relative flex items-center justify-center h-11 w-11 rounded-full bg-gray-50 transition-colors group-hover:bg-wh-brand/10 md:h-12 md:w-12">
                  {item.customIcon ? (
                    <span className="text-lg font-bold text-gray-900 group-hover:text-wh-brand-ink transition-colors">hp</span>
                  ) : (
                    <IconComponent className="relative z-10 w-[22px] h-[22px] text-gray-600 transition-colors group-hover:text-wh-brand-ink md:w-6 md:h-6" strokeWidth={1.75} />
                  )}
                  {item.badge && (
                    <motion.span
                      initial={{ scale: 0 }}
                ...
                <span className="text-[11px] font-semibold tracking-tight text-gray-500 transition-colors group-hover:text-wh-brand-ink text-center leading-tight md:text-sm md:text-slate-600">
                  {item.label}
                </span>
              </motion.button>
            </motion.div>
      ...
      <div className="px-4 py-6 max-w-lg mx-auto md:max-w-6xl md:px-8 md:py-8 md:w-full">
        ...
        >
          <div className="relative overflow-hidden rounded-[24px] bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-black/5 md:rounded-2xl md:p-6 md:shadow-sm md:ring-slate-200 md:border md:border-slate-200">
            <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-wh-brand/5 blur-3xl pointer-events-none md:bg-slate-100/80" />
            <div className="relative flex items-center gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[18px] bg-gradient-to-br from-wh-brand/10 to-wh-brand/5 ring-1 ring-wh-brand/10 md:from-slate-100 md:to-slate-50 md:ring-slate-200">
                <Store className="h-6 w-6 text-wh-brand-ink md:text-slate-700" strokeWidth={1.5} />
              </div>
              <div className="flex-1 min-w-0 text-left">
                <h2 className="text-[17px] font-extrabold text-gray-900 truncate tracking-tight mb-0.5 md:text-xl md:font-bold">
                  {sellerDisplayName}
                </h2>
                {sellerDisplayAddress && (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-base font-bold text-gray-900 truncate">
                        {loadingSeller ? "Loading..." : userData.name}
                      </h3>

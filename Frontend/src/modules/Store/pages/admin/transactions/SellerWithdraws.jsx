...
import { exportTransactionsToExcel, exportTransactionsToPDF } from "@store/components/admin/transactions/transactionsExportUtils"
...
export default function SellerWithdraws() {
  ...
  const handleExport = async (format) => {
    ...
    switch (format) {
      case "excel":
        exportTransactionsToExcel(exportData, headers, "seller_withdraws_full_details")
        break
      case "pdf":
        await exportTransactionsToPDF(exportData, headers, "seller_withdraws_full_details", "Seller Withdraws Report")
        break
      default: break
    }
  }
                  ...
                  {filteredWithdraws.length === 0 ? (
                    ...
                    filteredWithdraws.map((withdraw, index) => (
                      <tr key={withdraw.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        {visibleColumns.seller && <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-slate-700">{withdraw.sellerName || 'N/A'}</span>
                        </td>}
                        {visibleColumns.sellerId && <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-slate-700">{withdraw.sellerIdString || 'N/A'}</span>
                        </td>}
            ...
            {selectedWithdraw && (
              <div className="px-6 pb-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-900 mt-1">{selectedWithdraw.sellerName || 'N/A'}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase">Seller ID</label>
                  <p className="text-sm font-medium text-slate-900 mt-1">{selectedWithdraw.sellerIdString || 'N/A'}</p>
                </div>
                <div>
                ...
                <div className="border-t border-slate-200 pt-4">
                  ...
                  <div className="mt-2 space-y-2">
                    <p className="text-sm text-slate-800">
                      ...
                      {selectedWithdraw.sellerBankDetails?.accountHolderName || selectedWithdraw.sellerId?.accountHolderName || 'N/A'}
                    </p>
                    <p className="text-sm text-slate-800">
                      <span className="font-semibold">Account Number:</span>{' '}
                      {selectedWithdraw.sellerBankDetails?.accountNumber || selectedWithdraw.sellerId?.accountNumber || 'N/A'}
                    </p>
                    <p className="text-sm text-slate-800">
                      <span className="font-semibold">IFSC:</span>{' '}
                      {selectedWithdraw.sellerBankDetails?.ifscCode || selectedWithdraw.sellerId?.ifscCode || 'N/A'}
                    </p>
                    <p className="text-sm text-slate-800">
                      <span className="font-semibold">Account Type:</span>{' '}
                      {selectedWithdraw.sellerBankDetails?.accountType || selectedWithdraw.sellerId?.accountType || 'N/A'}
                    </p>
                    <p className="text-sm text-slate-800">
                      <span className="font-semibold">UPI ID:</span>{' '}
                      {selectedWithdraw.sellerBankDetails?.upiId || selectedWithdraw.sellerId?.upiId || 'N/A'}
                    </p>
                    {getSafeQrUrl(selectedWithdraw.sellerBankDetails?.upiQrImage || selectedWithdraw.sellerId?.upiQrImage) ? (
                      <div>
                        <p className="text-sm text-slate-800 font-semibold mb-2">UPI QR</p>
                        <img
                          src={getSafeQrUrl(selectedWithdraw.sellerBankDetails?.upiQrImage || selectedWithdraw.sellerId?.upiQrImage)}
                          alt="Seller UPI QR"

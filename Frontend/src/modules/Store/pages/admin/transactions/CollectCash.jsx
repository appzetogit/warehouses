...
import { emptyCollectCashTransactions } from "@store/utils/adminFallbackData"
...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter as DialogFooterComponent } from "@store/components/ui/dialog"
import { exportTransactionsToCSV, exportTransactionsToExcel, exportTransactionsToPDF, exportTransactionsToJSON } from "@store/components/admin/transactions/transactionsExportUtils"
...
export default function CollectCash() {
  ...
  const validateForm = () => {
    ...
    if (formData.type === "Seller" && !formData.seller) errors.seller = "Seller is required."
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      const newTransaction = {
        ...
        collectedFrom: formData.type === "Seller" ? "Hungry Puppets" : formData.deliveryman === "jhon-doe" ? "Jhon Doe" : "Leslie Alexander",
              ...
              {formData.type === "Seller" && (
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-4 py-2.5 border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm ${
                      formErrors.seller ? "border-red-500" : "border-slate-300"
                    }`}
                  >
                    <option value="">Select Seller</option>
                    <option value="hungry-puppets">Hungry Puppets</option>
                  </select>
                  {formErrors.seller && <p className="text-red-500 text-xs mt-1">{formErrors.seller}</p>}
                </div>
              )}

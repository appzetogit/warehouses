...
import { emptySellerVATReports, emptySellerVATStats } from "@store/utils/adminFallbackData"
...
export default function SellerVATReport() {
  const [searchQuery, setSearchQuery] = useState("")
  const [reports, setReports] = useState(emptySellerVATReports)
  ...
  const handleExport = (format) => {
    ...
    const headers = [
      { key: "sl", label: "SI" },
      { key: "sellerName", label: "Seller Info" },
      { key: "totalOrder", label: "Total Order" },
      { key: "totalOrderAmount", label: "Total Order Amount" },
      { key: "taxAmount", label: "Tax Amount" },
    ]
    switch (format) {
      case "csv": exportReportsToCSV(filteredReports, headers, "seller_vat_report"); break
      case "excel": exportReportsToExcel(filteredReports, headers, "seller_vat_report"); break
      case "pdf": exportReportsToPDF(filteredReports, headers, "seller_vat_report", "Seller VAT Report"); break
      case "json": exportReportsToJSON(filteredReports, "seller_vat_report"); break
    }
  }
  ...
  const activeFiltersCount = (filters.time !== "All Time" ? 1 : 0) + (filters.seller !== "All Sellers" ? 1 : 0)
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-blue-600">{emptySellerVATStats.totalOrders}</p>
                <p className="text-lg font-semibold text-blue-600 mt-1">{emptySellerVATStats.totalOrders}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-green-600">{emptySellerVATStats.totalOrderAmount}</p>
                <p className="text-lg font-semibold text-green-600 mt-1">{emptySellerVATStats.totalOrderAmount}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-red-600">{emptySellerVATStats.totalTaxAmount}</p>
                <p className="text-lg font-semibold text-red-600 mt-1">{emptySellerVATStats.totalTaxAmount}</p>
              </div>
                  ...
                  {filteredReports.map((report) => (
                    <tr key={report.sl} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          {report.icon && (
                            <img src={report.icon} alt={report.sellerName} className="w-8 h-8 rounded" />
                          )}
                          <span className="text-sm text-slate-700">{report.sellerName}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">

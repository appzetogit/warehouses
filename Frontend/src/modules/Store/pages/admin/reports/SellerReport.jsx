...
export default function SellerReport() {
  ...
  useEffect(() => {
    const fetchSellerReport = async () => {
      try {
        ...
        const response = await adminAPI.getSellerReport(params)
        ...
        debugError("Error fetching seller report:", error)
  ...
  const filteredSellers = useMemo(() => {
    return sellers // Backend already filters, so just return sellers
  }, [sellers])
  ...
  const handleExport = (format) => {
    ...
    const headers = [
      ...
      { key: "totalProduct", label: "Total Products" },
    ...
    switch (format) {
      case "csv": exportReportsToCSV(filteredSellers, headers, "seller_report"); break
      case "excel": exportReportsToExcel(filteredSellers, headers, "seller_report"); break
      case "pdf": exportReportsToPDF(filteredSellers, headers, "seller_report", "Seller Report"); break
      case "json": exportReportsToJSON(filteredSellers, "seller_report"); break
    }
  }
  ...
  return (
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Seller Report Table Section */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h2 className="text-xl font-bold text-slate-900">Seller Report Table {totalSellers}</h2>
                  ...
                  filteredSellers.map((seller) => (
                    <tr key={seller.sl} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-medium text-slate-700">{seller.sl}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-medium text-slate-900">{seller.totalOrderAmount}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-700">{seller.totalDiscountGiven}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`text-sm font-medium ${
                          seller.totalAdminCommission.startsWith('Rs.-') || seller.totalAdminCommission.startsWith('-Rs.')
                            ? 'text-red-600'
                            : 'text-slate-900'
                        }`}>
                          {seller.totalAdminCommission}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-700">{seller.totalVATTAX}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-700">{renderStars(seller.averageRatings, seller.reviews)}</span>
                      </td>
                    </tr>
                  ))
                )}

...
import { exportReviewsToCSV, exportReviewsToExcel, exportReviewsToPDF, exportReviewsToJSON } from "@store/components/admin/deliveryman/deliverymanExportUtils"
...
export default function SellerReviews() {
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter(review =>
      review.seller.toLowerCase().includes(query) ||
  ...
  const handleExport = (format) => {
    ...
    const exportRows = filteredReviews.map((review) => ({
      ...review,
      seller: review.seller || "N/A",
    }))
    switch (format) {
      case "csv": exportReviewsToCSV(exportRows, "seller_reviews", { subjectLabel: "Seller", subjectKey: "seller" }); break
      case "excel": exportReviewsToExcel(exportRows, "seller_reviews", { subjectLabel: "Seller", subjectKey: "seller" }); break
      case "pdf": exportReviewsToPDF(exportRows, "seller_reviews", { reportTitle: "Seller Reviews Report", subjectLabel: "Seller", subjectKey: "seller" }); break
      case "json": exportReviewsToJSON(exportRows, "seller_reviews"); break
    }
  }
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        setIsLoading(true)
        const response = await adminAPI.getSellerReviews({ limit: 1000 })
        ...
        debugError('Error fetching seller reviews:', error)
        setReviews([])
                ...
                <tbody className="bg-white divide-y divide-slate-100">
                  {filteredReviews.map((review) => (
                    <tr key={review.sl || review.orderId} className="hover:bg-slate-50 transition-colors">
                      ...
                      {visibleColumns.seller && <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-emerald-600">{review.seller}</td>}
          ...
          {selectedReview && (
            <div className="px-6 py-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-emerald-50 rounded-lg p-4"><p className="text-xs text-emerald-600 mb-1">Seller</p><p className="text-sm font-semibold text-emerald-700">{selectedReview.seller}</p></div>
              </div>
              <div className="grid grid-cols-2 gap-4">

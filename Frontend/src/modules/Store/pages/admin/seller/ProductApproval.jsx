...
export default function ProductApproval() {
  ...
  useEffect(() => {
    ...
    const onFocus = () => fetchProductRequests({ silent: true })
    const onVisibility = () => {
      if (document.visibilityState === "visible") {
        fetchProductRequests({ silent: true })
      }
    }
    const onPageShow = () => fetchProductRequests({ silent: true })
                        ...
                        <tr key={request._id || request.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-3 whitespace-nowrap">
                            <div className="text-sm">
                              <div className="font-semibold text-gray-900">{request.sellerName || '-'}</div>
                              <div className="text-gray-500 text-xs">{request.sellerId || '-'}</div>
                            </div>
                          </td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100/50 flex items-center justify-between">
                <div>
                   ...
                   <p className="text-sm font-semibold text-gray-900">{selectedRequest.sellerName || '-'}</p>
                   <p className="text-xs text-gray-500">ID: {selectedRequest.sellerId || '-'}</p>
                </div>

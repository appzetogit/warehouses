...
export default function UnregisteredSellers() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await adminAPI.getUnregisteredSellers();
      const list = response?.data?.data || [];
      setSellers(list);
    } catch (err) {
      console.error("Error fetching unregistered sellers:", err);
      setError(err.message || "Failed to fetch unregistered sellers");
      setSellers([]);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      setProcessing(true);
      await adminAPI.deleteUnregisteredSeller(selectedForDelete._id);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-extrabold text-slate-900 mt-2">{sellers.length}</h3>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-extrabold text-slate-900 mt-2">{filteredSellers.length}</h3>
            </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {loading ? (
                  ...
                  filteredSellers.map((request, index) => (
                    <tr key={request._id || index} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-semibold text-slate-900 block">{request.sellerName}</span>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-slate-100 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              ...
              <p className="text-sm text-slate-600 mb-6 leading-relaxed">
                Are you sure you want to permanently delete the lead inquiries for <strong className="text-slate-800">{selectedForDelete.sellerName}</strong> ({selectedForDelete.ownerName})?
              </p>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100/80 space-y-1">
                  ...
                  <span className="text-sm font-semibold text-slate-800 block break-words">{selectedForView.sellerName}</span>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100/80 space-y-1">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3 bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
                  <div className="w-9 h-9 rounded-lg bg-wh-brand-50 flex items-center justify-center text-wh-brand-ink shrink-0">

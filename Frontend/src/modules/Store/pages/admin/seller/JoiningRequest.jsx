...
export default function JoiningRequest() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      setError(err.message || "Failed to fetch seller requests")
      if (activeTab === "pending") {
        setPendingRequests([])
      } else {
        setRejectedRequests([])
  ...
  const handleApprove = async (request) => {
    if (window.confirm(`Are you sure you want to approve "${request.sellerName}" seller request?`)) {
      try {
        setProcessing(true)
        await adminAPI.approveSeller(request._id)
        
        // Refresh the list
        await fetchRequests()
        
        alert(`Successfully approved ${request.sellerName}'s join request!`)
      } catch (err) {
  ...
  const confirmReject = async () => {
    ...
    try {
      setProcessing(true)
      await adminAPI.rejectSeller(selectedRequest._id, rejectionReason)
      ...
      alert(`Successfully rejected ${selectedRequest.sellerName}'s join request!`)
    } catch (err) {
  ...
  const handleViewDetails = async (request) => {
    ...
    try {
      ...
      if (request.fullData) {
        ...
        setSellerDetails(request.fullData)
        setLoadingDetails(false)
        return
      }
      
      // Try to fetch full seller details from API
      const sellerId = request._id || request.id
      let response = null
      
      if (sellerId) {
        try {
          // Try admin API first
          if (adminAPI.getSellerById) {
            response = await adminAPI.getSellerById(sellerId)
          }
        } catch (err) {
          debugLog("Admin API failed, trying seller API:", err)
        }
        
        // Fallback to regular seller API
        if (!response || !response?.data?.success) {
          try {
            response = await sellerAPI.getSellerById(sellerId)
          } catch (err) {
            debugLog("Seller API also failed:", err)
          }
        }
      }
                            ...
                            <img
                              src={
                                ...
                                (typeof request.profileImage === "string"
                                  ...
                                  : (request.profileImage?.url || request.profileImageUrl?.url || request.sellerImage)) ||
                                "https://via.placeholder.com/40?text=" + (request.sellerName?.slice(0, 2) || "R").toUpperCase()
                              }
                              alt={request.sellerName || "Seller"}
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                e.target.src = "https://via.placeholder.com/40?text=" + (request.sellerName?.slice(0, 2) || "R").toUpperCase()
                              }}
                            />
                          </div>
                          <span 
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">{selectedRequest.sellerName}</p>
                </div>
              </div>
          ...
          >
            ...
            <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-5 flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-slate-900">Seller Details - {selectedRequest.sellerName || "N/A"}</h2>
              </div>
              <button
            ...
            <div className="p-6">
              ...
              {!loadingDetails && (sellerDetails || selectedRequest) && (() => {
                const r = sellerDetails || selectedRequest
                const sellerPhotoList = Array.isArray(r?.coverImages) ? r.coverImages.filter(Boolean) : []
                const profileImgUrl =
                  getNormalizedImageUrl(sellerPhotoList[0]) ||
                  (typeof r?.profileImage === "string" ? r.profileImage : (r?.profileImage?.url || r?.profileImageUrl?.url || r?.sellerImage))
                const addressParts = [
                  r?.addressLine1,
                  r?.addressLine2,
                  r?.area,
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 flex-wrap">
                        ...
                        <div className="flex items-center gap-1 text-slate-600">
                          ...
                          <span className="text-sm">{r?.sellerId || r?._id || "N/A"}</span>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  ...
                  {sellerPhotoList.length > 0 && (
                    <div className="pt-6 border-t border-slate-200">
                      <h4 className="text-lg font-semibold text-slate-900 mb-4">Seller Photos</h4>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {sellerPhotoList.map((sellerImg, idx) => {
                          const imgUrl = getNormalizedImageUrl(sellerImg)
                          return imgUrl ? (
                            <a
                              key={idx}
                              href={imgUrl}
                  ...
                  {(r?.createdAt || r?.sellerId || r?.businessModel || r?.approvedAt != null) && (
                    <div className="pt-6 border-t border-slate-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        {r.sellerId && (
                          <div>
                            ...
                            <p className="font-medium text-slate-900">{r.sellerId}</p>
                          </div>
                        )}
                        {r.approvedAt != null && (
                          <div>

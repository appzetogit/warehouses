...
export default function AllZonesMap() {
  ...
  // Draw zones and seller markers when map and data are ready
  useEffect(() => {
    if (!mapLoading && mapInstanceRef.current && window.google) {
      if (zones.length > 0 && sellers.length > 0) {
        drawAllZonesOnMap(window.google, mapInstanceRef.current)
      }
      if (sellers.length > 0) {
        drawSellerMarkers(window.google, mapInstanceRef.current)
      }
    }
  }, [zones, mapLoading, sellers])
  ...
  // Draw seller markers on the map
  const drawSellerMarkers = (google, map) => {
    ...
    sellers.forEach(seller => {
      ...
      // Get coordinates from seller location
      let lat = null
      let lng = null
      ...
      if (seller.location.coordinates && Array.isArray(seller.location.coordinates) && seller.location.coordinates.length >= 2) {
        ...
        lat = parseFloat(seller.location.latitude)
        lng = parseFloat(seller.location.longitude)
      }
      ...
      const infoWindow = new google.maps.InfoWindow({
        content: `
          <div style="padding: 12px; min-width: 200px;">
            ...
            <div style="font-size: 13px; color: #64748b; line-height: 1.6;">
              ${seller.location?.formattedAddress || seller.location?.address || seller.location?.area || 'Location not specified'}
            </div>
            ${seller.ownerName ? `
          ...
          {!mapLoading && (
            <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="text-xs text-slate-600 space-y-1">
                ...
                {sellers.length > 0 && (
                  <p>
                    Click on any <span className="font-semibold text-red-600">red marker</span> to view seller name and details. Total sellers: <strong>{sellers.length}</strong>
                  </p>
                )}
              </div>
            </div>

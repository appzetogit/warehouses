...
const PLAN_META = {
  starter: {
    label: "Starter",
    description: "For sellers with lower monthly GMV",
    icon: Award,
  ...
  premium: {
    label: "Premium",
    description: "For high monthly GMV sellers",
    icon: TrendingUp,
...
const SubscriptionSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true)
      const res = await adminAPI.getSellerSubscriptionSettings()
      try {
        ...
        const feature = featureRows.find((row) => row.key === "seller_subscription")
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true)
      const res = await adminAPI.updateSellerSubscriptionSettings(settings)
      if (res.data?.success) {

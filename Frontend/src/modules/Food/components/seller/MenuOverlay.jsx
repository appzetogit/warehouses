...
export default function MenuOverlay({ showMenu, setShowMenu }) {
  ...
  useEffect(() => {
    const checkAuth = () => {
      setIsAuthenticated(localStorage.getItem("seller_authenticated") === "true")
    }
    ...
    window.addEventListener('sellerAuthChanged', checkAuth)

    return () => {
      window.removeEventListener('storage', checkAuth)
      window.removeEventListener('sellerAuthChanged', checkAuth)
    }
  }, [])
  ...
  const getMenuOptions = () => {
    const baseOptions = [
      ...
      { id: 10, name: "Coupon", icon: Tag, route: "/seller/coupon" },
      { id: 11, name: "My Business Plan", icon: FileText, route: "/seller/business-plan" },
      { id: 12, name: "Reviews", icon: MessageSquare, route: "/seller/reviews" },
      { id: 14, name: "Wallet Method", icon: Settings, route: "/seller/wallet" },
      { id: 16, name: "Settings", icon: Settings, route: "/seller/settings" },
      { id: 17, name: "Conversation", icon: MessageCircle, route: "/seller/conversation" },
      { id: 18, name: "Privacy Policy", icon: Shield, route: "/seller/privacy" },
      { id: 19, name: "Terms & Condition", icon: CheckSquare, route: "/seller/terms" },
    ]
    ...
    if (isAuthenticated) {
      ...
      return [
        { id: 1, name: "Login", icon: LogIn, route: "/seller/login" },
        ...baseOptions
      ]
    }
  }

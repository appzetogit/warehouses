...
const SellerImageCarousel = React.memo(({ seller, priority = false, backendOrigin = "" }) => {
  ...
  const images = React.useMemo(() => {
    const sourceImages = Array.isArray(seller.images) && seller.images.length > 0
      ? seller.images
      : [seller.image];
  ...
  }, [seller.images, seller.image, withCacheBuster]);
  ...
  return (
    ...
    >
      <div className="relative">
        <SellerImageCarousel seller={seller} backendOrigin={backendOrigin} />
        <button
          onClick={(e) => {
            e.stopPropagation();
            onFavoriteClick(seller.id);
          }}
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-gray-100/80">
          <div className="flex items-center gap-1.5 text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
            ...
            <span className="text-[10px] sm:text-xs font-semibold">{seller.deliveryTime || "25-30 min"}</span>
          </div>
          <div className="flex items-center gap-1 text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
            ...
            <span className="text-[10px] sm:text-xs font-semibold">{seller.avgPrice || "₹200 for one"}</span>
          </div>
        </div>
      </div>
    </motion.div>

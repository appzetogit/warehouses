...
export default function DiningCategory() {
  ...
  useEffect(() => {
    const fetchSellers = async () => {
      try {
        setIsLoading(true)
        const response = await diningAPI.getSellers(
          category
        ...
        if (response?.data?.success) {
          const mapped = (Array.isArray(response.data.data) ? response.data.data : []).map((seller) => {
            const availability = getSellerAvailabilityStatus(seller)
            return {
              id: seller._id || seller.id,
              slug: seller.sellerNameNormalized || slugifySeller(seller.sellerName || seller.name),
              ...
              cuisine:
                Array.isArray(seller.cuisines) && seller.cuisines.length > 0
                  ...
                  : "Multi-cuisine",
              price: seller.costForTwo ? `Rs ${seller.costForTwo} for two` : "Price on request",
              rating: Number(seller.rating || seller.avgRating || 0).toFixed(1),
              offer: seller.offer || "Pre-book tables and dining offers",
              featuredDish: seller.featuredDish || "Chef's special",
              featuredPrice: seller.featuredPrice || null,
              availability,
            }
          })
          setSellers(mapped)
        ...
        setError("Failed to load dining sellers")
        setSellers([])
      } finally {
        setIsLoading(false)
      }
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <div className="mb-6 rounded-[28px] border border-[#f0dfca] bg-gradient-to-br from-[#fff4e7] via-white to-[#fff9f3] p-6 shadow-[0_18px_60px_rgba(90,55,20,0.08)] dark:border-gray-800 dark:bg-gradient-to-br dark:from-[#161616] dark:via-[#101010] dark:to-[#1a1a1a] dark:shadow-[0_18px_60px_rgba(0,0,0,0.35)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="inline-flex items-center gap-2 self-start rounded-full bg-white px-4 py-2 text-sm font-semibold text-[#6b5641] shadow-sm dark:border dark:border-gray-700 dark:bg-[#1a1a1a] dark:text-gray-300">
              ...
              <span>{sellers.length} places found</span>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
            {sellers.map((seller) => {
              const favorite = isFavorite(seller.slug)
              ...
              return (
                <Link
                  key={seller.id}
                  to={`/food/user/dining/${category}/${seller.slug}`}
                  state={{ seller }}
                >
                    ...
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute left-4 right-4 top-4 flex items-start justify-between gap-3">
                        <div className="rounded-full bg-black/60 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-sm">
                          {seller.featuredDish}
                          {seller.featuredPrice ? ` • ${"\u20B9"}${seller.featuredPrice}` : ""}
                        </div>
                        <Button
                          variant="ghost"
                          size="icon"
                      ...
                      <div className="absolute bottom-4 left-4 right-4">
                        ...
                        <p className="max-w-[85%] text-2xl font-black leading-tight text-white">{seller.offer}</p>
                      </div>
                    </div>
                    ...
                    <CardContent className="space-y-4 p-5">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <h2 className="truncate text-[22px] font-black leading-tight text-[#23180f] dark:text-white">{seller.name}</h2>
                          <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#6b5641] dark:text-gray-300">{seller.address}</p>
                        </div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#5f4c39] dark:text-gray-300">
                        ...
                        <span className="line-clamp-1">{seller.cuisine}</span>
                      </div>

                      <div className="flex flex-wrap gap-2">
                        <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${seller.availability?.isOpen ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300"}`}>
                          <Clock className="h-3.5 w-3.5" />
                          <span>{seller.availability?.isOpen ? "Open now" : "Closed now"}</span>
                        </div>
                        <div className="inline-flex items-center rounded-full bg-[#fff4e7] px-3 py-1.5 text-xs font-semibold text-[#a25b1f] dark:bg-[#2a1d12] dark:text-orange-300">
                          {formatTimingLabel(seller.availability)}
                        </div>
                      </div>

                      <div className="flex items-center justify-between border-t border-dashed border-[#ead7c0] pt-4 dark:border-gray-700">
                        <div className="text-sm font-semibold text-[#4c3b2c] dark:text-gray-200">{seller.price}</div>

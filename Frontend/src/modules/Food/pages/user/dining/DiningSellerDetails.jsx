...
const formatAddress = (seller) =>
  ...
  [seller?.location?.area || seller?.area, seller?.location?.city || seller?.city]
    .filter(Boolean)
    .join(", ")

const buildImageList = (seller) => {
  const candidates = [
    seller?.coverImage?.url,
    seller?.coverImage,
    ...(Array.isArray(seller?.coverImages) ? seller.coverImages.map((image) => image?.url || image) : []),
    ...(Array.isArray(seller?.menuImages) ? seller.menuImages.map((image) => image?.url || image) : []),
...
const buildFacilities = (seller) => {
  const facilities = []

  if (seller?.diningSettings?.tableBookingEnabled !== false) facilities.push("Dinner")
  if (seller?.isAcceptingOrders !== false) facilities.push("Lunch")
  if (seller?.diningSettings?.homeDeliveryAvailable || seller?.homeDeliveryAvailable) facilities.push("Home delivery")
  if (seller?.diningSettings?.takeawayAvailable || seller?.takeawayAvailable) facilities.push("Takeaway available")
  if (seller?.diningSettings?.vegOnly || seller?.vegOnly) facilities.push("Vegetarian only")
  if (seller?.diningSettings?.lessNoisy || seller?.ambience === "quiet") facilities.push("Less noisy")
...
export default function DiningSellerDetails() {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      try {
        ...
        const routeSeller = location.state?.seller || null
        const preferredSellerLookup =
          routeSeller?._id ||
          routeSeller?.sellerId ||
          routeSeller?.id ||
          slug

        const sellerResponse = await sellerAPI.getSellerById(preferredSellerLookup)
        ...
        const sellerId = resolvedSeller?._id || resolvedSeller?.id || slug
        const menuResponse = await sellerAPI.getMenuBySellerId(sellerId).catch(() => null)
  ...
  const address = formatAddress(seller) || "Address unavailable"
  const imageGallery = buildImageList(seller)
  ...
  const costForTwo = seller?.costForTwo ? `${"\u20B9"}${seller.costForTwo} for two` : `${"\u20B9"}1900 for two`
  const facilities = buildFacilities(seller)
  const rating = Number(seller?.rating || seller?.avgRating || 0).toFixed(1)
  const reviewCount = seller?.totalRatings || seller?.reviewCount || seller?.reviewsCount || 0
  const openingTime = formatTimeLabel(seller?.openingTime || seller?.diningSettings?.openingTime || "12:00")
  const closingTime = formatTimeLabel(seller?.closingTime || seller?.diningSettings?.closingTime || "23:59")
  const isDiningEnabled = seller?.diningSettings?.isEnabled !== false
  const topTabs = [
    { id: "prebook", label: "Pre-book offers", target: "seller-prebook" },
    { id: "walkin", label: "Walk-in offers", target: "seller-prebook" },
    { id: "menu", label: "Menu", target: "seller-menu" },
    { id: "photos", label: "Photos", target: "seller-photos" },
    { id: "about", label: "About", target: "seller-about" },
  ]
  ...
  const sellerFavoriteSlug =
    seller?.sellerNameNormalized ||
    seller?.slug ||
    slug

  const favorite = isFavorite(sellerFavoriteSlug)
  ...
  const handleToggleFavorite = () => {
    if (favorite) {
      removeFavorite(sellerFavoriteSlug)
      return
    }

    addFavorite({
      slug: sellerFavoriteSlug,
      name: sellerName,
      cuisine: cuisines,
      rating,
      image: heroImage,
    ...
    <div className="min-h-screen bg-[#f6f7fb] pb-28">
      <section className="mx-auto max-w-md bg-[#f6f7fb]">
        <div className="relative h-[392px] overflow-hidden">
          {heroImage ? (
            <img src={heroImage} alt={sellerName} className="h-full w-full object-cover" />
          ) : (
          ...
          <div className="absolute inset-x-0 bottom-0 px-3 pb-4 text-white">
            <div className="flex items-end justify-between gap-3">
              <div className="min-w-0 flex-1">
                <h1 className="text-[36px] font-black leading-none tracking-[-0.03em]">{sellerName}</h1>
      ...
      <div className="mx-auto max-w-md px-4 pt-4">
        ...
        <section id="seller-menu" className="mt-5 border-t border-[#e8e8ef] pt-4">
          <div className="flex items-end justify-between gap-3">
            <div>
        ...
        <section id="seller-photos" className="mt-5 border-t border-[#e8e8ef] pt-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {(imageGallery.length > 0 ? imageGallery.slice(0, 4) : menuPreviewImages.slice(0, 2)).map((image, index) => (
              ...
              >
                {image ? (
                  <img src={image} alt={`${sellerName} ${index + 1}`} className="h-full w-full object-cover" />
                ) : (
        ...
        <section id="seller-about" className="mt-5 border-t border-[#e8e8ef] pt-4">
        ...
        <div className="fixed inset-0 z-40">
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-[28px] bg-white px-4 pb-6 pt-4 shadow-[0_-20px_60px_rgba(15,23,42,0.18)]">
            ...
            <div className="grid grid-cols-4 gap-3">
              {Array.from({ length: Math.min(seller?.diningSettings?.maxGuests || 6, 8) }, (_, index) => index + 1).map((count) => (
                <button
                  key={`sheet-${count}`}

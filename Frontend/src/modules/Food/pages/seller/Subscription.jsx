  ...
  const toggle = async () => {
    ...
    if (next && transactions === null) {
      try {
        ...
        const response = await sellerAPI.getSubscriptionInvoice(invoice._id)
    ...
    const fetchAll = async () => {
      try {
        ...
        const [overviewRes, invoicesRes, timelineRes] = await Promise.allSettled([
          sellerAPI.getSubscriptionOverview(),
          sellerAPI.getSubscriptionInvoices({ limit: 50 }),
          sellerAPI.getSubscriptionTransactions({ limit: 50 }),
        ])

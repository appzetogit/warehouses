...
const parseSellerTimeToParts = (value) => {
...
const buildNextBusinessDayResumeAt = (sellerProfile) => {
  const now = new Date()
  const openDays = Array.isArray(sellerProfile?.openDays)
    ? sellerProfile.openDays.map(normalizeDayName).filter(Boolean)
    : []
  const openingTime = parseSellerTimeToParts(
    sellerProfile?.openingTime || "09:00",
  )
...
const buildStockRule = ({
  ...
  if (selectedOption === "next-business-day") {
    return {
      ...
      createdAt,
      resumeAt: buildNextBusinessDayResumeAt(sellerProfile),
    }
  }
  ...
  useEffect(() => {
    const fetchSellerProfile = async () => {
      try {
        ...
        debugWarn("Failed to load seller profile for stock rules:", error)
      }
    }

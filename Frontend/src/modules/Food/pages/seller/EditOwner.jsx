...
export default function EditOwner() {
  ...
  // Fetch seller data from backend on mount
  useEffect(() => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (profileImageFile) {
        try {
          const imageResponse = await sellerAPI.uploadProfileImage(profileImageFile)
      ...
      const response = await sellerAPI.updateProfile(updatePayload)
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Sign out from Firebase if seller logged in via Google
      try {
      ...
      // Clear seller module authentication data
      clearModuleAuth("seller")
      
      // Clear all seller-related localStorage data
      ...
      localStorage.removeItem("seller_invited_users")

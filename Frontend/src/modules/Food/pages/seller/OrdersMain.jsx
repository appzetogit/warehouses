...
import { getSellerCookingNote } from "@food/utils/orderCookingNote";
import BottomNavOrders from "@food/components/seller/BottomNavOrders";
import SellerNavbar from "@food/components/seller/SellerNavbar";
import NewOrderAcceptCard from "@food/components/seller/NewOrderAcceptCard";
import { sellerAPI, diningAPI } from "@food/api";
import { useSellerNotifications } from "@food/hooks/useSellerNotifications";
import ResendNotificationButton from "@food/components/seller/ResendNotificationButton";
import {
...
const STORAGE_KEY = "seller_online_status";
...
const getQueuedOrderKeys = (orderLike = {}) =>
  [
    ...
    orderLike?._id,
    orderLike?.id,
    getSellerOrderAlertKey(orderLike),
  ]
...
const transformOrderForList = (order) => ({
  ...
  note: getSellerCookingNote(order),
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          // Filter cancelled orders (both seller and user cancelled)
...
function TableBookings() {
  ...
  useEffect(() => {
    let isMounted = true;
    let sellerEntity = null;

    const resolveSellerEntity = async () => {
      if (sellerEntity) return sellerEntity;
      const res = await sellerAPI.getCurrentSeller();
      sellerEntity =
        res.data?.data?.seller || res.data?.seller || res.data?.data || null;
      return sellerEntity;
    };

    const fetchBookings = async () => {
      try {
        const seller = await resolveSellerEntity();
        const sellerId = seller?._id || seller?.id;

        if (sellerId) {
          const response = await diningAPI.getSellerBookings(seller);
  ...
  const handleStatusUpdate = async (bookingId, status) => {
    try {
      const response = await diningAPI.updateBookingStatusSeller(bookingId, status);
...
function AllOrders({ onSelectOrder, onCancel, searchQuery = "" }) {
  ...
  const handleMarkReady = async ({ orderId, mongoId }) => {
    ...
    try {
      ...
      await sellerAPI.markOrderReady(orderKey);
      setOrders((prev) =>
        prev.map((order) =>
...
export default function OrdersMain() {
  ...
  const normalizeIncomingOrder = (orderLike = {}) => ({
    ...
    total:
      ...
      0,
    note: getSellerCookingNote(orderLike),
    paymentMethod:
  ...
  // Seller notifications hook for real-time orders
  const { newOrder, clearNewOrder } = useSellerNotifications();
  ...
  useEffect(() => {
    const fetchSellerStatus = async () => {
      try {
        ...
        const seller =
          response?.data?.data?.seller || response?.data?.seller;
        if (seller) {
          setSellerStatus({
          ...
          try {
            if (seller.operationalStatus) {
              setOnlineStatus(
                seller.operationalStatus.isEffectivelyOnline
                  ? "Online"
                  : "Offline",
              );
              if (
                typeof seller.operationalStatus.isEffectivelyOnline ===
                "boolean"
              ) {
                localStorage.setItem(
                  STORAGE_KEY,
                  JSON.stringify(
                    Boolean(seller.operationalStatus.isEffectivelyOnline),
                  ),
                );
              }
            } else {
              ...
              if (savedStatus !== null) {
                ...
                setOnlineStatus(
                  seller.isAcceptingOrders ? "Online" : "Offline",
                );
              }
            }
          } catch {
        ...
        if (
          ...
          debugError("Error fetching seller status:", error);
        }
        // Set loading to false so UI doesn't stay in loading state
        setSellerStatus((prev) => ({ ...prev, isLoading: false }));
      }
    };
    ...
    window.addEventListener("sellerProfileRefresh", handleProfileRefresh);
  ...
  const handleReverify = async () => {
    try {
      ...
      window.dispatchEvent(new Event("sellerProfileRefresh"));
      ...
      if (
        ...
        debugError("Error reverifying seller:", error);
      }
      ...
      if (error.response?.status === 401) {
        ...
        const errorMessage =
          error.response?.data?.message ||
          "Failed to reverify seller. Please try again.";
        alert(errorMessage);
      }
    } finally {
  ...
  // Lenis fights the seller layout scroll containers on desktop — keep native scroll.
  ...
  useEffect(() => {
    ...
    window.addEventListener("seller:new_order", onCustomEvent);
    return () => {
      window.removeEventListener("seller:new_order", onCustomEvent);
    };
  }, [clearNewOrder]);
  ...
  // Unlock shared seller alert audio on first gesture (session owns playback).
  useEffect(() => {
    const unlockAudio = () => {
        ...
        if (response.data?.success && response.data.data?.orders) {
          ...
          for (const orderToQueue of sorted) {
            const orderForQueue = {
              ...
              sellerId: orderToQueue.sellerId,
              sellerName: orderToQueue.sellerName,
              ...
              note: getSellerCookingNote(orderToQueue),
  ...
  const handleAcceptQueuedOrder = async (orderToAccept, prepTime = 11) => {
    ...
    try {
      ...
      await sellerAPI.acceptOrder(orderId, prepTime);
      debugLog("? Order accepted:", orderId);
      toast.success("Order accepted successfully");
      stopSellerAlert(orderToAccept);
  ...
  const handleRejectConfirm = async () => {
    ...
    if (orderToReject?.orderMongoId || orderToReject?.orderId) {
      try {
        ...
        await sellerAPI.rejectOrder(orderId, rejectReason);
  ...
  const handlePrintOrder = async (orderToPrint) => {
    ...
    try {
      ...
      doc.text(orderToPrint.sellerName || "Seller", 105, 30, {
        align: "center",
      });
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      await sellerAPI.rejectOrder(orderId, cancelReason.trim());
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gray-100 md:bg-slate-50 md:overflow-hidden overflow-x-hidden">
      {/* Seller Navbar - Mobile only */}
      <div className="sticky top-0 z-50 bg-white md:hidden">
        <SellerNavbar showNotifications={true} />
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col md:flex-row md:gap-6 px-4 md:px-6 pt-0 md:pt-6 pb-24 md:pb-6 md:overflow-hidden">
        ...
        <div
          ...
          {!sellerStatus.isLoading &&
            !sellerStatus.isActive &&
            sellerStatus.onboarding?.completedSteps === 4 && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3, delay: 0.1 }}
                className={`mt-4 mb-4 rounded-2xl shadow-sm px-6 py-4 ${sellerStatus.rejectionReason
        ...
        if (remainingMinutes <= 0 && order.status === "preparing") {
          ...
          if (elapsedSeconds >= totalETASeconds - 2) {
            try {
              ...
              await sellerAPI.markOrderReady(

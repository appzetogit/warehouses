...
import OrderDetailPage from "@food/pages/seller/OrderDetailPage"
const debugLog = (...args) => {}
...
const ORDER_STATUS_API_MAP = {
  ...
  delivered: "delivered",
  rejected: "cancelled_by_seller",
  cancelled: "cancelled_by_user,cancelled_by_seller,cancelled_by_admin",
}
...
export default function AllOrdersPage() {
  ...
  const { newOrder } = useSellerNotifications()
    ...
    const fetchSellerData = async () => {
      try {
        ...
        if (err.response?.status !== 401) {
          debugError('Error fetching seller data:', err)
        }
      }
    }
    fetchSellerData()
  ...
  const transformOrder = useCallback((order) => {
    ...
    // Get seller name
    const sellerName = sellerData?.name || order.sellerId?.name || 'Seller'
    ...
    if (status === 'REJECTED' && order.rejectionReason) {
      ...
      reason = `Cancelled by ${order.cancelledBy === 'customer' ? 'customer' : 'seller'}: ${order.cancellationReason}`

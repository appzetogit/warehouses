        ...
        <div className="flex-1">
          ...
          {Number(order.discount || 0) > 0 && (
            <p className="mt-1 text-[11px] font-medium text-rose-600">
              Discount ₹{Number(order.sellerDiscountShare || order.discount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              {Number(order.adminDiscountShare || 0) > 0
                ...
                : ""}
              {Number(order.sellerDiscountShare || 0) > 0 && Number(order.discount || 0) > Number(order.sellerDiscountShare || 0)
...
export default function HubFinance() {
  ...
  const isSellerSubscriptionEnabled = financeData?.features?.sellerSubscriptionEnabled !== false
  // Locked = total outstanding calendar-month subscription dues; the full balance stays visible.
  const subscriptionDueAmount = Number(financeData?.subscription?.lockedAmount ?? financeData?.seller?.subscriptionDueAmount ?? 0)
  ...
  useEffect(() => {
    const fetchSubscriptionHistory = async () => {
      try {
        ...
        const response = await sellerAPI.getSubscriptionTransactions({ limit: 20 })
  ...
  // Fetch seller data for header display
  useEffect(() => {
    // Use seller data from financeData if available, otherwise fetch separately
    if (financeData?.seller) {
      setSellerData(financeData.seller)
    } else {
      const fetchSellerData = async () => {
        try {
          const response = await sellerAPI.getSellerByOwner()
          const data = response?.data?.data?.seller || response?.data?.seller || response?.data?.data
          if (data) {
            setSellerData({
              name: data.name,
              sellerId: data.sellerId || data._id,
  ...
  // Format seller ID to REST###### format (e.g., REST005678)
  const formatSellerId = (sellerId) => {
    if (!sellerId) return ''
  ...
  const invoiceSummary = useMemo(() => {
    const earnings = invoiceOrders.reduce((sum, order) => sum + (order.payout || order.sellerEarning || 0), 0)
  ...
  const handleViewDetails = () => {
    navigate("/seller/finance-details", { state: { financeData, sellerData } })
  }
  ...
  const getReportData = () => {
    const sellerName = financeData?.seller?.name || "Seller"
    const sellerId = financeData?.seller?.sellerId || "N/A"
    ...
    return `
      ...
      <body>
        <div class="header">
          ...
          <p>${reportData.sellerName}</p>
          <p>ID: ${reportData.sellerId}</p>
      ...
      return `
              ...
              <tfoot>
                <tr style="background-color: #e8f5e9; font-weight: bold;">
                  ...
                  <td colspan="2">₹${reportData.allOrders.reduce((sum, order) => sum + (order.payout || order.sellerEarning || 0), 0).toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          ` : `
        ...
        <div class="footer">
          ...
          <p>Total Orders: ${reportData.allOrders?.length || 0} | Total Earnings: ₹${reportData.allOrders?.reduce((sum, order) => sum + (order.payout || order.sellerEarning || 0), 0).toFixed(2) || '0.00'}</p>
        </div>
      </body>
      </html>
    `
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <p className="text-lg font-bold text-gray-900 truncate">
                  {sellerData?.name || financeData?.seller?.name || "Seller"}
                </p>
                <ChevronDown className="w-4 h-4 text-gray-600 flex-shrink-0" />
              </div>
              <p className="text-xs text-gray-600 mt-0.5">
                {(() => {
                  const sellerId = sellerData?.sellerId || financeData?.seller?.sellerId
                  const address = sellerData?.address || financeData?.seller?.address || ''
                  const parts = []
                  if (sellerId) {
                    const formattedId = formatSellerId(sellerId)
      ...
      <div className="hidden md:flex shrink-0 items-end justify-between gap-6 px-8 pt-7 pb-5 border-b border-[#d5dbe3] bg-[#f2f4f7]/90 backdrop-blur">
        <div>
          ...
          <p className="mt-1 text-sm text-[#5c6775]">
            {sellerData?.name || financeData?.seller?.name || "Seller"} · settle earnings and track every rupee
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-6 pb-28 md:px-8 md:pt-7 md:pb-8 md:min-h-0">
        {activeTab === "payouts" && (
          <div className="space-y-6 md:grid md:grid-cols-[minmax(280px,340px)_minmax(0,1fr)] md:gap-8 md:space-y-0 md:items-start">
            ...
            {isSellerSubscriptionEnabled && subscriptionDueAmount > 0 && (
              <motion.div
                  ...
                  <>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const hasDues = isSellerSubscriptionEnabled && subscriptionDueAmount > 0;
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const response = await sellerAPI.createWithdrawalRequest(amount)
                        if (response.data?.success) {
                          // Professional success toast or similar would go here
                          setShowWithdrawalModal(false)
                          setWithdrawalAmount('')
                          // Refresh finance data
                          const financeResponse = await sellerAPI.getFinance()
                          if (financeResponse.data?.success && financeResponse.data?.data) {
                            setFinanceData(financeResponse.data.data)
                          }
                          const withdrawalResponse = await sellerAPI.getWithdrawalHistory()

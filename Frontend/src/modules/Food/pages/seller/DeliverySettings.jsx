...
export default function DeliverySettings() {
  ...
  // The actual Online/Offline override switch lives on Seller status.
  useEffect(() => {
    let cancelled = false

    const loadDeliveryStatus = async () => {
      try {
        const [sellerRes, timingsRes] = await Promise.all([
          sellerAPI.getCurrentSeller(),
          sellerAPI.getOutletTimings().catch(() => null),
        ])
        const seller =
        ...
        const outletTimings =
          ...
          seller?.outletTimings ||
          null
        const operational =
          seller?.operationalStatus ||
          getSellerOperationalStatus(
            { ...seller, outletTimings, outsideHoursOverride: false },
            new Date(),
          )
        if (!cancelled) {
  ...
  const saveDeliveryStatusToBackend = async (status) => {
    ...
    try {
      ...
      await sellerAPI.updateAcceptingOrders(nextStatus)
    } catch (error) {

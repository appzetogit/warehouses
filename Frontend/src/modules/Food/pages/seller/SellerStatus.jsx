...
import useSellerBackNavigation from "@food/hooks/useSellerBackNavigation"
...
} from "@food/utils/sellerOperationalStatus"
const debugLog = (...args) => {}
const debugError = (...args) => {}

const buildSellerForAvailability = (seller, outletTimings, isAcceptingOrders) => ({
  ...(seller || {}),
  isAcceptingOrders: Boolean(isAcceptingOrders),
  outsideHoursOverride: false,
  outletTimings: outletTimings || seller?.outletTimings || null,
})

const syncEffectiveOnlineStatus = (seller, outletTimings, isAcceptingOrders) => {
  const operational = getSellerOperationalStatus(
    buildSellerForAvailability(seller, outletTimings, isAcceptingOrders),
  )
  broadcastSellerOperationalStatus(operational)
  return operational
}
...
export default function SellerStatus() {
      ...
      try {
        ...
        if (data) {
          ...
          if (data.operationalStatus) {
            broadcastSellerOperationalStatus(data.operationalStatus)
          } else {
  ...
  useEffect(() => {
    ...
    syncEffectiveOnlineStatus(sellerData, outletTimings, deliveryStatus)
  }, [sellerData, outletTimings, deliveryStatus, currentDateTime])

  // Check if seller is currently open based on outlet timings only
  useEffect(() => {
    const checkIfOpen = () => {
      if (typeof document !== "undefined" && document.hidden) return
      const now = new Date()
  ...
  const handleDeliveryStatusChange = async (checked) => {
    ...
    try {
      try {
        const response = await sellerAPI.updateAcceptingOrders(checked)
        const updated =
          response?.data?.data?.seller || response?.data?.seller || null
        if (updated) {
          setSellerData(updated)
          if (updated.outletTimings) setOutletTimings(updated.outletTimings)
          if (updated.operationalStatus) {
            broadcastSellerOperationalStatus(updated.operationalStatus)
          } else {
          ...
          syncEffectiveOnlineStatus(sellerData, outletTimings, checked)
        }
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <h2 className="text-base font-bold text-gray-900 mb-1">
                  {loading ? "Loading..." : (sellerData?.name || "Seller")}
                </h2>
                <p className="text-sm text-gray-500">
                  {loading ? "Loading..." : (
                    <>
                      {sellerData?.id ? `ID: ${String(sellerData.id).slice(-5)}` : ""}
                      {sellerData?.location && formatAddress(sellerData.location) ? (
                        <> | {formatAddress(sellerData.location)}</>
                      ) : ""}
                    </>
                  )}
                </p>
  ...
  {!isWithinTimings && sellerData && !isDayClosed && (

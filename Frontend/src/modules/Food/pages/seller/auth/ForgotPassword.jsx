...
export default function SellerForgotPassword() {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await sellerAPI.sendOTP(null, "reset-password", email)
      setStep(2)
      setResendTimer(60)
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await sellerAPI.verifyOTP(null, otpCode, "reset-password", null, email)
      setStep(3)
    } catch (err) {
      const message =
        err?.response?.data?.message ||
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      const response = await sellerAPI.resetPassword(email, otp.join(""), newPassword)

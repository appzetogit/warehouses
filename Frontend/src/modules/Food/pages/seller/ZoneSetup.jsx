...
import { sellerAPI, zoneAPI } from "@food/api"
...
export default function ZoneSetup() {
  ...
  const hasHydratedMapFromSellerRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (lastValidLocationRef.current) {
      ...
      const locationForFocus =
        isPending && sellerData?.pendingLocation
          ? sellerData.pendingLocation
          : sellerData?.location
  ...
  }, [serviceZones, mapLoading, sellerData])

  // Load existing seller location once when map + profile are ready.
  ...
  useEffect(() => {
    if (hasHydratedMapFromSellerRef.current) return
    if (userHasSelectedLocationRef.current) {
      hasHydratedMapFromSellerRef.current = true
      return
    }
    if (!sellerData || mapLoading || !mapInstanceRef.current || !window.google) return
  ...
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      const hasPublishedLocation = Boolean(getSavedLocationCoords(sellerData?.location))
      ...
      if (response?.data?.data?.seller) {
        ...
        setSellerData(response.data.data.seller)
  ...
  const locationUpdateStatus = String(sellerData?.locationUpdateStatus || "none").toLowerCase()
  const hasPendingLocationUpdate = locationUpdateStatus === "pending"
  const pendingLocationLabel =
    sellerData?.pendingLocation?.formattedAddress ||
    sellerData?.pendingLocation?.address ||
    formatAddress(sellerData?.pendingLocation) ||
    ""

...
export default function HubMenu() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    try {
      const saved = localStorage.getItem("seller_hub_menu_active_tab")
  ...
  // Seller info - fetch from backend
  const sellerName = sellerData?.name || ""
  const isPureVegSeller = sellerData?.pureVegSeller === true
  const sellerExpertise = sellerData?.cuisines?.length > 0 
    ? sellerData.cuisines.join(", ") 
    : ""
  ...
  const fetchMenu = async (showLoading = true) => {
    try {
      ...
      const response = await sellerAPI.getMenu()
  ...
  const fetchAddons = async (showLoading = true) => {
    try {
      ...
      const response = await sellerAPI.getAddons()
  ...
  useEffect(() => {
    try {
      localStorage.setItem("seller_hub_menu_active_tab", activeTab)
    } catch (error) {
  ...
  const handleSaveAddon = async () => {
    ...
    try {
          ...
          try {
            ...
            try {
              uploadResponse = await uploadAPI.uploadMedia(file, {
                folder: 'switcheats/seller/addons'
              })
      ...
      if (editingAddon) {
        ...
        await sellerAPI.updateAddon(editingAddon.id, { draft: addonData })
        toast.success('Add-on updated successfully! Pending admin approval.')
      } else {
        // Create new add-on
        await sellerAPI.addAddon(addonData)
  ...
  const handleEditAddon = (addon) => {
    ...
    setAddonFoodType(isPureVegSeller ? "veg" : (addon.foodType === "non-veg" ? "non-veg" : "veg"))
  ...
  const handleDeleteAddon = async (addon) => {
    ...
    try {
      await sellerAPI.deleteAddon(addon.id)
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await sellerAPI.bulkUploadTemplate()
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      setIsUploading(true)
      const response = await sellerAPI.bulkUpload(bulkUploadFile)
  ...
  const handleAvailabilityConfirm = async () => {
    ...
    try {
      ...
      if (switchingOffTarget.type === 'item') {
        ...
        await sellerAPI.updateFood(switchingOffTarget.id, payload)
        ...
        await Promise.all(itemIds.map((itemId) => sellerAPI.updateFood(itemId, payload)))
        setMenuData((prev) =>
          prev.map((entry) =>
  ...
  const handleSaveCategoryName = () => {
    ...
    // Save updated foods
    try {
      localStorage.setItem('seller_foods', JSON.stringify(updatedFoods))
              ...
              <AnimatePresence mode="wait">
                {!isScrolled ? (
                  ...
                  <motion.h1
                    ...
                    title={sellerName}
                  >
                    {sellerName.length > 25 ? `${sellerName.substring(0, 25)}...` : sellerName}
                  </motion.h1>
                )}
              </AnimatePresence>
            </div>
          ...
          <AnimatePresence>
            {!isScrolled && (
              ...
              >
                ...
                <p className="text-sm text-gray-600">{sellerExpertise}</p>
              </motion.div>
            )}
          </AnimatePresence>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <button
                            onClick={async () => {
                              try {
                                await sellerAPI.updateAddon(addon.id, { isAvailable: addon.isAvailable === false })
                      ...
                      <div key={item.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                        ...
                        {isPendingApproval(item.approvalStatus) && (
                          <div className="flex items-center justify-center gap-3 mt-4">
                            <button
                              onClick={() => navigate(`/seller/hub-menu/item/${item.id}`, { state: { item, groupId: group.id } })}
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className={`grid gap-2 ${isPureVegSeller ? "grid-cols-1" : "grid-cols-2"}`}>
                    <button
                      type="button"

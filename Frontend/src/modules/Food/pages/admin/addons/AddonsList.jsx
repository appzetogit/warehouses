...
export default function AddonsList() {
  ...
  useEffect(() => {
    const fetchAddons = async () => {
      try {
        setLoading(true)
        const response = await adminAPI.getSellerAddons({
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await adminAPI.updateSellerAddon(String(id), {
  ...
  const confirmDelete = async () => {
    ...
    try {
      setSubmittingAction(true)
      await adminAPI.rejectSellerAddon(String(id), "Deleted by admin")
              ...
              {loading ? (
                ...
                filteredAddons.map((addon, index) => (
                  <tr key={String(addon.id || addon._id)} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-sm text-slate-900">{addon?.seller?.name || "-"}</span>
                        {addon?.seller?.ownerPhone ? (
                          <span className="text-xs text-slate-500">{addon.seller.ownerPhone}</span>
                        ) : null}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm bg-slate-50 border border-slate-200 rounded-lg p-4">
                <p>
                  ...
                  <span className="text-slate-900">{selectedAddon?.seller?.name || "-"}</span>
                </p>
                <p>

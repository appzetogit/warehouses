...
function ActionDialog({ action, invoice, onClose, onDone }) {
  ...
  const config = {
    deduct: {
      ...
      description: `Deduct part or all of the outstanding due (${formatMoney(outstanding)}) directly from the seller's wallet balance. The locked amount is released as it is settled.`,
      needsAmount: true,
      confirmLabel: "Deduct",
      danger: false,
    },
    markPaid: {
      title: "Mark as Paid",
      description: `Record a payment the seller made outside the platform (cash / bank transfer). Leave amount empty to settle the full outstanding of ${formatMoney(outstanding)}.`,
      needsAmount: true,
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <p className="text-xs text-slate-500 mt-1">
          {invoice.seller?.sellerName || ""} • {invoice.billingMonthLabel || invoice.billingMonth}
        </p>
      ...
      <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-xs text-slate-500">{invoice?.seller?.sellerName || ""}</p>
          </div>
...
export default function SubscriptionHistory() {
  ...
  const handleRunBilling = async () => {
    ...
    if (!window.confirm(`Run billing for ${billingMonth}? Already-invoiced sellers are skipped (safe to re-run).`)) return;
    try {
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-slate-900">{invoice.seller?.sellerName || "-"}</p>
                    <p className="text-[10px] text-slate-500">{invoice.seller?.ownerPhone || ""}</p>
                  </td>

...
import { adminAPI, sellerAPI, uploadAPI } from "@food/api"
...
import sellerIcon from "@food/assets/Dashboard-icons/image2.png"
...
const zoneLabelFromSeller = (seller, zones = []) => {
  const zid = seller?.zoneId
  const zoneName =
...
const mapRawSeller = (seller, index, zones) => ({
  ...
  zone: zoneLabelFromSeller(seller, zones),
  approvalStatus: normalizeApprovalStatus(seller),
  isActive: seller.isActive !== false && seller.status === "approved",
  rating: seller.rating || seller.ratings?.average || 0,
  logo: getPrimarySellerImage(seller, PLACEHOLDER_40),
  originalData: seller,
})
...
const normalizeApprovalStatus = (seller) => {
  const raw = String(seller?.status || "").trim().toLowerCase()
...
// Order matters: profileImage is the seller's own identity image and the one an admin
...
const getPrimarySellerImage = (seller, fallback = "") => {
  ...
  return (
    ...
    normalizeImageUrl(seller?.sellerImage) ||
...
export default function SellersList() {
  ...
  const [sellerStats, setSellerStats] = useState({ total: 0, active: 0, inactive: 0 })
  const [page, setPage] = useState(1)
  const [selectedSeller, setSelectedSeller] = useState(null)
  const [sellerDetails, setSellerDetails] = useState(null)
  const [sellerOutletTimings, setSellerOutletTimings] = useState(null)
  const [loadingDetails, setLoadingDetails] = useState(false)
  const [banConfirmDialog, setBanConfirmDialog] = useState(null) // { seller, action: 'ban' | 'unban' }
  ...
  // Format Seller ID to REST format (e.g., REST422829)
  ...
  useEffect(() => {
    ...
    const fetchSellers = async () => {
      try {
        ...
        const rawList = Array.isArray(data?.sellers)
          ? data.sellers
          : Array.isArray(data)
            ? data
        ...
        if (rawList.length > 0 || body?.success === true) {
          const mappedSellers = rawList.map((seller, index) =>
            mapRawSeller(seller, index, zones)
          )
          if (!cancelled) {
            setSellers(mappedSellers)
            setTotalSellers(Number(data?.total) || mappedSellers.length)
            if (data?.stats) {
              setSellerStats({
          ...
          setSellers([])
          setTotalSellers(Number(data?.total) || 0)
        }
      } catch (err) {
        if (cancelled) return
    ...
    const t = setTimeout(fetchSellers, 250)
    return () => {
      cancelled = true
      clearTimeout(t)
    }
  ...
  const totalPages = Math.max(1, Math.ceil(totalSellers / PAGE_SIZE))
  const showingFrom = totalSellers === 0 ? 0 : (page - 1) * PAGE_SIZE + 1
  const showingTo = Math.min(page * PAGE_SIZE, totalSellers)
  ...
  useEffect(() => {
    if (sellerIdFromUrl) {
      handleViewDetails({ _id: sellerIdFromUrl, id: sellerIdFromUrl })
    }
  }, [sellerIdFromUrl])

  const filteredSellers = useMemo(() => {
    if (sortConfig.key !== "zone") return sellers
  ...
  const activeSellers = sellerStats.active
  const inactiveSellers = sellerStats.inactive
  ...
  const getLocationFromSeller = (seller) => {
    return (
      seller?.onboarding?.step1?.location ||
      seller?.location ||
      seller?.originalData?.location ||
      {}
    )
  }
  ...
  const normalizeLocationFormFromSeller = (seller) => {
    const loc = getLocationFromSeller(seller)
    ...
    return {
      zoneId: seller?.zoneId || seller?.location?.zoneId || "",
      latitude: latitude || "",
      longitude: longitude || "",
  ...
  const handleViewDetails = async (seller) => {
    ...
    try {
      ...
      if (!sellerId || !adminAPI.getSellerById) {
        setSellerDetails(seller.originalData || seller)
        return
      }

      const [detailsRes, timingsRes] = await Promise.all([
        adminAPI.getSellerById(sellerId).catch(() => null),
        sellerAPI.getOutletTimingsBySellerId(sellerId, { noCache: true }).catch(() => null),
      ])

      const detailsData = detailsRes?.data?.data
      if (detailsData && (detailsData.sellerName || detailsData._id)) {
        setSellerDetails(detailsData)
      } else {
        setSellerDetails(seller.originalData || seller)
      }
      ...
      if (outletTimings && typeof outletTimings === "object" && !Array.isArray(outletTimings)) {
        setSellerOutletTimings(outletTimings)
      }
    } catch (err) {
      debugError("Error fetching seller details:", err)
      // Use the seller data we already have
      setSellerDetails(seller.originalData || seller)
    } finally {
  ...
  const handleEditLocation = async (seller) => {
    await handleViewDetails(seller)
  ...
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      const response = await adminAPI.updateSellerLocation(sellerId, locationPayload)
      const updatedSeller = response?.data?.data?.seller

      if (updatedSeller?.location) {
        ...
        setSellers((prev) =>
          prev.map((item) =>
            (item._id === sellerId || item.id === sellerId)
              ? {
                ...item,
                zone:
                  updatedSeller.location.area ||
                  updatedSeller.location.city ||
                  item.zone,
                originalData: {
                  ...(item.originalData || {}),
                  location: updatedSeller.location,
      ...
      debugError("Error saving seller location:", err)
      alert(err?.response?.data?.message || "Failed to update seller location")
    } finally {
  ...
  useEffect(() => {
    if (!isEditingLocation || !selectedSeller) return

    const sourceSeller = sellerDetails || selectedSeller?.originalData || selectedSeller
    const initialForm = normalizeLocationFormFromSeller(sourceSeller)
  ...
  }, [isEditingLocation, selectedSeller, sellerDetails?._id])
  ...
  const getDetailsEditSource = () => {
    return sellerDetails || selectedSeller?.originalData || selectedSeller || null
  }

  const buildDetailsFormFromSeller = (seller) => {
    if (!seller) {
      return {
        name: "",
        pureVegSeller: false,
    ...
    const openingTimeValue =
      ...
      seller.onboarding?.step2?.deliveryTimings?.openingTime ||
      ""
    const closingTimeValue =
      seller.closingTime ||
      seller.deliveryTimings?.closingTime ||
      seller.onboarding?.step2?.deliveryTimings?.closingTime ||
      ""
    const estimatedDeliveryTimeValue =
      seller.estimatedDeliveryTime ||
      seller.onboarding?.step4?.estimatedDeliveryTime ||
      ""
    ...
    return {
      ...
      primaryContactNumber: seller.primaryContactNumber || seller.ownerPhone || "",
      email: seller.email || seller.ownerEmail || "",
  ...
  const handleStartEditDetails = () => {
    ...
    setDetailsForm(buildDetailsFormFromSeller(source))
    setProfileImageFile(null)
    setProfileImagePreview(getPrimarySellerImage(source))
    setIsEditingLocation(true)
    ...
    try {
      ...
      if (profileImageFile) {
        const uploadRes = await uploadAPI.uploadMedia(profileImageFile, {
          folder: "switcheats/seller/profile",
        })
      ...
      const response = await adminAPI.updateSeller(sellerId, payload)
      const updatedSeller = response?.data?.data?.seller

      if (updatedSeller) {
        setSellerDetails(updatedSeller)
        setSellers((prev) =>
          prev.map((item) =>
            (item._id === sellerId || item.id === sellerId)
              ? {
                ...item,
                name: updatedSeller.name || item.name,
                ownerName: updatedSeller.ownerName || item.ownerName,
                ownerPhone: updatedSeller.ownerPhone || updatedSeller.phone || item.ownerPhone,
                zone: updatedSeller.location?.area || updatedSeller.location?.city || item.zone,
                isActive: updatedSeller.isActive !== false,
                approvalStatus: normalizeApprovalStatus(updatedSeller),
                logo: getPrimarySellerImage(updatedSeller, item.logo),
                originalData: {
                  ...(item.originalData || {}),
                  ...updatedSeller,
                },
      ...
      debugError("Error updating seller details:", err)
  ...
  // Handle ban/unban seller
  const handleBanSeller = (seller) => {
    const isBanned = !seller.isActive
    setBanConfirmDialog({
      seller,
  ...
  const confirmBanSeller = async () => {
    ...
    const { seller, action } = banConfirmDialog
    ...
    try {
      ...
      try {
        ...
        setSellers(prevSellers =>
          prevSellers.map(r =>
            r.id === seller.id || r._id === seller._id
        ...
        debugLog(`Seller ${isBanning ? 'banned' : 'unbanned'} successfully`)
      } catch (apiErr) {
        ...
        setBanConfirmDialog(null)
        alert(`Seller ${isBanning ? 'banned' : 'unbanned'} locally. Please check backend connection.`)
      }

    } catch (err) {
      debugError("Error banning/unbanning seller:", err)
  ...
  const confirmDeleteSeller = async () => {
    ...
    try {
      ...
      try {
        ...
        // Show success message
        alert(`Seller "${seller.name}" deleted successfully!`)
      } catch (apiErr) {
        debugError("API Error:", apiErr)
        alert(apiErr.response?.data?.message || "Failed to delete seller. Please try again.")
      }

    } catch (err) {
      debugError("Error deleting seller:", err)
  ...
  const loadSellersExportUtils = () =>
    import("@food/components/admin/sellers/sellersExportUtils")

  const handleExport = async () => {
    const dataToExport = filteredSellers.length > 0 ? filteredSellers : sellers
    const filename = "sellers_list"
    const utils = await loadSellersExportUtils()
    utils.exportSellersToPDF(dataToExport, filename)
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 p-4 lg:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{sellerStats.total || totalSellers}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{activeSellers}</p>
              </div>
              <div className="w-12 h-12 rounded-lg bg-green-100 flex items-center justify-center">
                <img src={sellerIcon} alt="Seller" className="w-8 h-8" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{inactiveSellers}</p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <DropdownMenuContent align="end" className="w-56 bg-white border border-slate-200 rounded-lg shadow-lg z-50 animate-in fade-in-0 zoom-in-95 duration-200 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95">
                  ...
                  <DropdownMenuItem 
                    onClick={async () => {
                      ...
                      const filename = "sellers_list"
                      const utils = await loadSellersExportUtils()
                      utils.exportSellersToExcel(dataToExport, filename)
                    }} 
          ...
          <div className="text-sm text-slate-600 mb-4">
            {loading
              ? "Loading..."
              : `Showing ${showingFrom}-${showingTo} of ${totalSellers} sellers`}
          </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-slate-500">ID #{formatSellerId(seller.originalData?.sellerId || seller.originalData?._id || seller._id || seller.id)}</span>
                              <span className="text-xs text-slate-500">{renderStars(seller.rating)}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-500">{formatPhone(seller.ownerPhone)}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col gap-1">
                            <span className={`inline-flex w-fit items-center rounded-full px-2.5 py-1 text-xs font-semibold ${approvalStatusBadgeClass(seller.approvalStatus)}`}>
                              {approvalStatusLabel(seller.approvalStatus)}
                            </span>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded transition-colors ${!seller.isActive
                                ? "text-green-600 hover:bg-green-50"
                                : "text-red-600 hover:bg-red-50"
                                }`}
                              title={!seller.isActive ? "Unban Seller" : "Ban Seller"}
                            >
          ...
          {!loading && totalSellers > PAGE_SIZE ? (
        ...
        >
          ...
          >
            ...
            <div className="p-8 overflow-y-auto">
              ...
              {!loadingDetails && !isEditingDetails && (sellerDetails || selectedSeller) && (() => {
                const r = sellerDetails || selectedSeller?.originalData || selectedSeller
                const detailsApprovalStatus = normalizeApprovalStatus(r)
                const profileImgUrl = getPrimarySellerImage(r)
                ...
                <div className="space-y-10">
                  ...
                  <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                    <div className="w-32 h-32 rounded-3xl overflow-hidden bg-slate-50 shrink-0 shadow-inner group">
                      <img
                        ...
                        alt={r?.sellerName || r?.name || "Seller"}
                    ...
                    <div className="flex-1 text-center md:text-left pt-2">
                      ...
                      <div className="flex items-center justify-center md:justify-start gap-6 flex-wrap">
                        ...
                        <div className="flex items-center gap-2 text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                          ...
                          <span className="text-xs font-bold tracking-wider">{formatSellerId(r?.sellerId || r?._id)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium text-slate-900">
                                {r?.location ? formatLocationAddress(r.location, selectedSeller?.zone) : flatAddress}
                              </p>
                            </div>
                          </div>
                        )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        {r.sellerId && (
                          <div>
                            ...
                            <p className="font-medium text-slate-900">{formatSellerId(r.sellerId)}</p>
                          </div>
                        )}
                        {r.slug && (
                          <div>
                  ...
                  {r?.onboarding?.step1 && (
                    <div className="pt-6 border-t border-slate-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        {r.onboarding.step1.sellerName && (
                          <div>
                            <p className="text-xs text-slate-500 mb-1">Seller Name (at registration)</p>
                            <p className="font-medium text-slate-900">{r.onboarding.step1.sellerName}</p>
                          </div>
                        )}
                  ...
                  {(r?.slug || r?.sellerId || r?.phoneVerified !== undefined || r?.signupMethod) && (
                    <div className="pt-6 border-t border-slate-200">
              ...
              {!loadingDetails && !sellerDetails && !selectedSeller && (
                <div className="flex flex-col items-center justify-center py-20">
      ...
      {banConfirmDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={cancelBanSeller}>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">
                    {banConfirmDialog.action === 'ban' ? 'Ban Seller' : 'Unban Seller'}
                  </h3>
                  <p className="text-sm text-slate-600">
                    {banConfirmDialog.seller.name}
                  </p>
                </div>
              </div>
              ...
              <p className="text-sm text-slate-700 mb-6">
                {banConfirmDialog.action === 'ban'
                  ? 'Are you sure you want to ban this seller? They will not be able to receive orders or access their account.'
                  : 'Are you sure you want to unban this seller? They will be able to receive orders and access their account again.'
                }
              </p>

              <div className="flex items-center gap-3">
                <button
                  onClick={cancelBanSeller}
                  disabled={banning}
      ...
      {deleteConfirmDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={cancelDeleteSeller}>
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    {deleteConfirmDialog.seller.name}
                  </p>
                </div>
              </div>

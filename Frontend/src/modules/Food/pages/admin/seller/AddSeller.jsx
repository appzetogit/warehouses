...
const ADMIN_ADD_STORAGE_KEY = "admin_add_seller_form_data"
const ADMIN_ADD_FILES_DB = "AdminAddSellerFiles"
...
export default function AddSeller() {
  ...
  const checkDuplicateSellerPhone = async (phoneNumber = "") => {
    const normalized = sanitizeDigits(phoneNumber).slice(-10)
    if (!normalized || normalized.length !== 10) return false
    try {
      const res = await adminAPI.getSellers({ limit: 1000 })
      const list = res?.data?.data?.sellers || res?.data?.sellers || []
      return list.some((seller) => {
        const owner = sanitizeDigits(String(seller?.ownerPhone || "")).slice(-10)
        const primary = sanitizeDigits(String(seller?.primaryContactNumber || "")).slice(-10)
  ...
  const handleNext = async () => {
    ...
    if (step === 1) {
      const duplicateOwnerPhone = await checkDuplicateSellerPhone(step1.ownerPhone)
      if (duplicateOwnerPhone) {
        const msg = "Seller already exists with the same mobile number"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (step2.profileImage instanceof File) {
        profileImageData = await handleUpload(step2.profileImage, "switcheats/seller/profile")
      ...
      for (const file of step2.menuImages.filter(f => f instanceof File)) {
        const uploaded = await handleUpload(file, "switcheats/seller/menu")
      ...
      if (step3.panImage instanceof File) {
        panImageData = await handleUpload(step3.panImage, "switcheats/seller/pan")
      ...
      if (step3.gstRegistered && step3.gstImage) {
        if (step3.gstImage instanceof File) {
          gstImageData = await handleUpload(step3.gstImage, "switcheats/seller/gst")
      ...
      if (step3.fssaiImage instanceof File) {
        fssaiImageData = await handleUpload(step3.fssaiImage, "switcheats/seller/fssai")
      ...
      // Call backend API
      const response = await adminAPI.createSeller(payload)
      ...
      debugError("Error creating seller:", error)
      const errorMsg = error?.response?.data?.message || error?.message || "Failed to create seller. Please try again."
          ...
          <div>
            ...
            <Input
              ...
              onBlur={async () => {
                const duplicateOwnerPhone = await checkDuplicateSellerPhone(step1.ownerPhone)
                if (duplicateOwnerPhone) {
                  setFormErrors((prev) => ({ ...prev, ownerPhone: "Seller already exists with the same mobile number" }))
                }
              }}
              className="mt-1 bg-white text-sm text-black placeholder-black"
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <footer className="px-4 sm:px-6 py-3 bg-white">
        <div className="flex justify-between items-center">
          ...
          >
            {step === 3 ? (isSubmitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Creating... </> : "Create Seller") : isSubmitting ? "Saving..." : "Continue"}
          </Button>
        </div>
      </footer>

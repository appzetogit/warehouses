...
export default function SellerCommission() {
  ...
  const filteredCommissions = useMemo(() => {
    ...
    return commissions.filter(commission =>
      ...
      commission.seller?.name?.toLowerCase().includes(query)
    )
  ...
  const fetchCommissions = async () => {
    try {
      setLoading(true)
      const response = await adminAPI.getSellerCommissions({})
  ...
  const fetchApprovedSellers = async () => {
    try {
      const response = await adminAPI.getApprovedSellers({ limit: 1000 })
      
      let sellersData = null
      if (response?.data?.success && response?.data?.data?.sellers) {
        sellersData = response.data.data.sellers
      ...
      if (sellersData && Array.isArray(sellersData)) {
        setApprovedSellers(sellersData)
      } else {
        setApprovedSellers([])
      }
    } catch (error) {
      debugError('Error fetching approved sellers:', error)
      
      // Handle network errors silently (already handled in fetchCommissions)
      if (error.code !== 'ERR_NETWORK' && error.message !== 'Network Error') {
        toast.error(error.response?.data?.message || 'Failed to fetch approved sellers')
      }
    }
  }
  ...
  const handleEdit = async (commission) => {
    try {
      ...
      if (commissionData) {
        ...
        // Handle seller ID - it can be an object with _id or just an ID string
        let sellerId = ""
        if (commissionData.seller) {
          ...
          // Fallback to sellerId field if seller object is not populated
          sellerId = commissionData.sellerId || commissionData.seller || ""
        }
                        ...
                        {visibleColumns.seller && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-medium text-blue-600">
                              {commission.sellerName || commission.seller?.name || '-'}
                            </span>
                          </td>
                        )}
                        {visibleColumns.sellerId && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm text-slate-700">{commission.sellerId || '-'}</span>
                          </td>
                        )}
                        {visibleColumns.defaultCommission && (
                          <td className="px-6 py-4 whitespace-nowrap">
        ...
        <DialogContent className="max-w-xl bg-white p-0">
          ...
          <div className="space-y-4 px-6 py-4">
            ...
            <div className="max-h-80 overflow-y-auto space-y-2">
                ...
                .map((seller) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-sm text-slate-900">{seller.name}</p>
              ...
              {filteredSellers.filter(r => !r.hasCommissionSetup).length === 0 && (
          ...
          <div className="space-y-4 px-6 py-4">
            ...
            {selectedSeller && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                <p className="font-semibold text-sm text-slate-900">{selectedSeller.name}</p>
                <p className="text-xs text-slate-600 mt-0.5">{selectedSeller.sellerId}</p>
              </div>
            )}
        ...
        <DialogContent className="max-w-md bg-white">
          ...
          <div className="py-4">
            <p className="text-sm text-slate-700">
              Are you sure you want to delete commission for "{selectedCommission?.sellerName || selectedCommission?.seller?.name}"? This action cannot be undone.
            </p>
          </div>
          <DialogFooter>
            <button
